...
│   │   ├── agentic_cell.py      #   autonomous ReAct cell: run_agentic_cell_node (one visit = one iteration; route_after_cell loop-back = per-iteration Rewind-able checkpoint) + 3 strict-schema tools (run_terminal/read_file_ast/apply_granular_edit) + audit_tool_args (DANGEROUS + secret scrub) + contained MCTS (select_candidate_via_mcts: transactional push→verify→rollback, verdict-as-reward; only live brain.mcts edge) + leak-safe session registry + _chunk_hook tee field (_CellSession) for real-time PTY streaming to the WS dispatcher
│   │   ├── cell_dispatcher.py   #   CellEventDispatcher Protocol (glass-box typed events: tool_call_start / pty_chunk / ast_diff / governor_tick) + NullCellDispatcher silent no-op default; brain/ only — no api/ import; LiveCellDispatcher lives in api/websocket_manager.py, injected via config["configurable"] seam
...
│   │   ├── permissions.py      #   3-axis matrix (SessionPermissionMode × ToolPrivilegeTier × AgentIdentity) + RBWE guard (Phase 5.1) + classify_tool_privilege fail-closed verb heuristic + register_privilege_overrides catalog hook
│   │   ├── mcp_constants.py    #   shared ALLOWED_MCP_COMMANDS launch-command allowlist (core-layer policy fact; consumed by api/mcp_servers + mcp_registry)
│   │   ├── mcp_registry.py     #   curated regulated-server SSoT (RegulatedServer install metadata + per-tool tiers; build_privilege_catalog / init_registry derive the permission catalog)
│   │   ├── mcp_config.py       #   .ailienant/config.json projection: export_mcp_config (uri-credential redaction + key_ref placeholders) / import_mcp_config (idempotent name-keyed reconcile, allowlist-guarded, version fail-fast)
│   │   ├── tool_rag.py         #   RAM LanceDB schema store + select_tools(intent, k=5) (Phase 5.2)
│   │   ├── sandbox.py          #   SandboxAdapter ABC (+ supports_sessions / open_session session contract) + DockerSandboxAdapter (kernel-side `timeout`; --read-only, --network none, ro bind-mount, tmpfs /work; + _DockerPtyBackend persistent-shell exec socket) + NativeHITLSandboxAdapter (degraded host-spawn gated by `vfs_manager.request_human_approval`, `SANDBOX_DEGRADED_EXEC` sentinel, asyncio.wait_for + process.kill+reap, DLQ stub) + WasmSandboxAdapter (wasmtime WASI pure-compute; 5M-instruction fuel cap, no preopens, ADR-002 module-import Scope Guard / WasmScopeError) + NativeDirectSandboxAdapter (persistent-PTY tier, defined-but-dormant — not yet in the resolver) + resolve_default_adapter() startup probe (Docker→Wasm→NativeHITL degradation ladder; ACTIVE_TIER/ACTIVE_ADAPTER globals + get_active_tier getter) (Phase 6.1.1 + 6.1.2 + 6.1.3 + 6.1.4)
│   │   ├── pty_session.py      #   SandboxSession ABC + _PtySession persistent interactive shell (one long-lived shell owns cwd/env; AsyncIterator[bytes] streamed deltas, write_stdin, interrupt=Ctrl-C / kill=process-tree teardown+reap) over a reader-thread→asyncio.Queue bridge (event loop never blocks; lossless backpressure) + Unix-openpty (echo-disabled) / pywinpty-ConPTY / degraded-pipe backends
│   │   ├── workspace_sync.py   #   Bidirectional VFS↔Sandbox sync: SyncSurface ABC + LocalFsSyncSurface (rglob+sha256) + DockerSyncSurface (O(1) latency: single exec_run sha256sum, put_archive/get_archive) + push_vfs_to_surface (O(1) memory: blob_store on-demand) + pull_surface_to_vfs (three-way diff changed/deleted/unchanged, OCC guard + ghost-deletion detection); get_sync_surface() on Docker+NativeDirect adapters
...
│   │   ├── shims/              #   webview build shims — js-yaml-stub.ts (fail-fast stub aliased over js-yaml in the workspace bundle; the diff viewer drags the full parser only for its unused structural-YAML mode)
...
│   │   │   │                   #     + CellAuditWidget (Glass-Box agentic-cell audit log — collapsible accordion per iteration: tool_call → sanitized PTY output → AST diff + budget-governor footer; PTY panel row-virtualized via useWindowedRows; fed by the 4 server_cell_* deltas into transient Message.cellRun; the LIVE iteration's panel is interactive — flicker-free auto-scroll (useLayoutEffect + stick-to-bottom) + a PtyStdinBar that posts PTY_STDIN → client_pty_write into the session's write_stdin)
│   │   │   │                   #     + ExecutionChecklist (progressive WBS task list ☐→🔄→✅/✗ driven by server_graph_mutation per-step status flips; seeded by server_plan_document early-emit; durable audit evidence — in PERSIST_TRANSCRIPT, survives window reload; memoized on step count + status signatures)
...
│   │   │   │                   #     + PlanPanel (legacy rich Plan surface: structured MissionSpecification w/ WBS rows + clickable blue file-links that OPEN_FILE in the editor; superseded by PlanAcceptancePanel)
│   │   │   │                   #     + PlanAcceptancePanel (Plan-mode split surface: "Accept this plan?" with selectable WBS preview + three-way decision — auto-accept / manual-approve / keep-planning — wired to SUBMIT_TASK with explicit execution_mode; chat composer folds away while active)
...
│   │   │   #   utils/sanitizePty — strip ANSI escape sequences + collapse \r overwrites for the static cell-audit log (full xterm.js terminal is a separate surface)
│   │   │   #   utils/useWindowedRows — fixed-height row virtualization scoped to a local scroll container (reads scrollTop/clientHeight from a bound ref, never window)
...
│   └── esbuild.js              #   3 build contexts: extension (CJS), webview (IIFE), dashboard (ESM+splitting); 550 KB webview ceiling + shiki-leak sentinels; WEBVIEW_ALIAS stubs js-yaml off the IIFE
├── docs/
...
│   ├── PHASE_7_19_BLUEPRINT.md # Master architectural contract for Phase 7.19 — Agentic Execution Cell & Persistent Audit Trail (ADR-747..755)
