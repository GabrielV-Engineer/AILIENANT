...
│   │   │   │                   #     + CellAuditWidget (Glass-Box agentic-cell audit log — collapsible accordion per iteration: tool_call → sanitized PTY output → AST diff + budget-governor footer; PTY panel row-virtualized via useWindowedRows; fed by the 4 server_cell_* deltas into transient Message.cellRun)
