...
│   │   └── test/               #   vscode-test mocha suite — persistedStore.test.ts (Phase 7.11.2 — rAF-coalesce + rehydrate round-trip + version-mismatch safe discard, 3 tests) + streamingMarkdownParser.test.ts (Phase 7.11.5 — W1 O(1) flag-delta audit + W9 CommonMark §4.5 nested-fence symmetry + source-buffer immutability + 7 others, 10 tests) + workspacePathIndex.test.ts (Phase 7.11.4 — trie round-trip + 500 ms debounce + folder-cap bail-out + extractMentions dedup, 5 tests)
