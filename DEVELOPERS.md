...
- **Researcher and Orchestrator tool bundles are built; graph promotion is the remaining step.** Researcher's five READ_ONLY tools (glob, grep, workspace_structure, query_graphrag, get_dependents) and Orchestrator's introspection tools (get_wbs_status, emit_hitl_request) are built and role-gated; their state-injecting factories are wired. Researcher's output is still consumed as optional Planner context rather than from a standalone graph node, and the Orchestrator's operations are still direct state access. Promotion of both to first-class graph nodes rides on the DEBT-066 dispatch loop. The Analyst (six READ_ONLY tools: run_linter, analyze_complexity, audit_dependencies, diff_changes, web_search, read_token_ledger), Planner, and Coder are the fully-wired cognitive core.
- **The 56-tool role-gated catalog is built; the cognitive dispatch loop is not yet wired.** Six tool waves shipped ~34 net-new role-gated tools (Researcher, Analyst, Orchestrator, Planner, Coder, Gateway/Benchmark, Universal) on top of the original 16, bringing the registered store to ~50 tools with `tool_search` deferred-loading at the 10%-budget gate. Schema definitions, state-injecting factories, and RBAC assignments are all present and correct. The remaining gap is cognitive activation: no LLM tool-dispatch loop yet invokes the registered orchestrator/coder/analyst tools from within a graph node (DEBT-066, targeted for a future Agency sprint); factories are ready and correct the moment dispatch lands.
