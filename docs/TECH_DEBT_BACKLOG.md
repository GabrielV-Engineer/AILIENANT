...
| DEBT-122 | Rich Tool Chips (`ToolChip`/`ActionLog`) and the agentic-cell audit (`CellAuditWidget`) render as unmigrated siblings below `AgentTimeline`, not as timeline rows — no backend activity marker emits `ref = tool_call_id`/an iteration ref yet, since Rich Tool Chips is a separate mechanism from the coder's `run_command` narration path that feeds `server_activity_event`. `upsertToolBody` (frontend) is implemented and unit-tested, ready once that marker exists. | MEDIUM | Feature gap | 11.5.C.4 (future) | Floating |
