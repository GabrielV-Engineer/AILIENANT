...
### DEBT-235 [MEDIUM · Floating] — Natt (Analyst) has no access to session activity; only sees the frontend-attached active file, never what the Coder/Companion/planner just did

- **Date:** 2026-09-01
- **Files:** `ailienant-core/agents/analyst_context.py` (assemble_analyst_context, the 5-source budgeted pipeline), `ailienant-core/core/task_service.py:2138-2181` (stream_analyst_reply, the natt: memory namespace), `ailienant-core/core/memory/semantic_memory.py` (SemanticMemoryManager — the vector store this gap's fix would extend), `ailienant-core/brain/state.py` (WBSStep, AIlienantGraphState.messages), `ailienant-core/brain/checkpoint.py` (HybridCheckpointer — confirmed insufficient as a source, see below), `ailienant-core/core/telemetry_log.py` (confirmed insufficient as a source, see below), `ailienant-extension/src/providers/workspace_panel.ts:1169-1181` (client_analyst_query, where context_paths and any future session-activity flag would be added).
- **Capability (not built) + why the obvious sources don't already cover it:** Natt's memory is deliberately namespaced (`mem_key = f"natt:{session_id}"`, task_service.py:2181) and separate from the main chat by design — it never sees planner/coder messages, WBS steps, or applied diffs unless the frontend explicitly attaches a file path as `context_paths`. Two candidate sources were investigated and found insufficient: `core/telemetry_log.py` is write-only (a RotatingFileHandler sink, no read/query/filter API) with content truncated to 2000 chars/line (`_LINE_CAP`) — built for human `tail -f`, not programmatic retrieval. The LangGraph checkpoint's `messages` field (`brain/state.py:506`) has full-fidelity content but is periodically collapsed by `StateSummarizer` (`brain/summarizer.py:128-175`) to stay within context budget — detail from earlier in a long session is not reliably still there. No session-scoped search function (`search_session_activity` or equivalent) exists anywhere in the codebase (confirmed by exhaustive grep).
- **The requested capability, as designed with the user:** an opt-in toggle in the Natt pane (same precedent as `enable_native_thinking`, ws_contracts.py:676) that, when active, lets the user ask Natt about anything that happened in the current session — not just the last turn. Backing mechanism: extend `SemanticMemoryManager` (core/memory/semantic_memory.py) with a session-scoped namespace (today's `_WORKSPACE_SCHEMA` only isolates by `workspace_hash`; would need a `session_id` field plus write hooks at the four event sources below) rather than reusing the code-only GraphRAG table. Four event types get indexed as they occur: (1) diffs/code the Coder applies (apply_gate.py/checks_gate.py), (2) Coder Companion reasoning (CoderCompanionPayload's decisions/patterns_applied/reasoning_summary, ailienant-extension/src/api/contracts.ts:513-533), (3) main-chat messages (AIlienantGraphState.messages, brain/state.py:506), (4) WBS steps (WBSStep, brain/state.py:73-112). Retrieval reuses the same semantic-search pattern `fetch_intent_snippets` already uses for GraphRAG (agents/analyst_context.py:60-84), scoped to `session_id` instead of `workspace_hash`, with its own char cap (same pattern as CODEX_CAP/FILE_CAP/RAG_CAP/DOCS_CAP in analyst_context.py:49-54). Scope is ephemeral — session-only, discarded on session close, consistent with the existing `natt:` memory isolation and the checkpoint's own in-memory L1 tier.
- **Why it is not a defect:** nothing shipped claims Natt has session awareness; the current four-brain design (Codex/README/GraphRAG/docs + active file) was a deliberate, well-executed scope boundary, not an oversight — this is a scope expansion, not a bug.
- **What it would take:** (1) a new LanceDB table/namespace keyed by session_id (schema design, cleanup-on-close policy since L1 checkpoint is also memory-only), (2) four write hooks at the event sources listed above, (3) a retrieval function mirroring fetch_intent_snippets scoped to session_id, (4) a new opt-in flag threaded through client_analyst_query (ws_contracts.py) -> stream_analyst_reply (task_service.py) -> assemble_analyst_context (analyst_context.py) as a sixth budgeted source, (5) frontend toggle UI in the Natt pane. A real feature slice — not a quick patch.
- **Phase:** unscheduled — candidate for a future Natt/session-context division.
- **Notes:** the underlying mechanism (a session-scoped, ephemeral semantic index) is not Natt-specific — it is general session-activity infrastructure that other consumers could reuse once it exists: the Coder/Planner querying it for continuity within a long session ("did I already try this?", "what pattern did I use two steps ago?"), session-state recovery if a crash/restart outruns the in-memory L1 checkpoint, or a future dashboard session-audit view (strictly better than grepping the truncated `.ailienant_telemetry.log`). This entry scopes the gap to the Natt consumer only — extending it to other consumers is a separate design decision (shared schema ownership, write-order, TTL semantics across consumers) not evaluated here.
...
### DEBT-201 [LOW · Floating] — HITL routing-gate prompt is not suppressed for a single-model BYOM preset

A user who wants only one model for everything can already pin it today by setting all four BYOM preset tiers (`small`/`medium`/`big`/`cloud`) to the same model — a UX shortcut for this ("Use for all" per tier row) shipped in `BYOMPanel.tsx`'s preset editor. But `brain/routing_gate.py::run_model_route_node` has no awareness of this: it still suspends for a routing-confirmation prompt in `SessionPermissionMode.CAUTIOUS`/`PLAN_ONLY` sessions (`_GATE_MODES`) even when every tier the router could pick resolves to the identical model, asking the operator to confirm or override a choice that is functionally moot. AUTO/STANDARD sessions already bypass the gate unconditionally (`session_mode not in _GATE_MODES`), which narrows real-world impact, but CAUTIOUS/PLAN_ONLY users on a single-model preset still see the prompt every turn. A fix must short-circuit only the *interrupt* inside `run_model_route_node` (e.g. compare the four resolved `ModelTarget.model` strings once per preset-apply/cache-refresh) and must NOT skip `core/memory/context_auditor.py`'s upstream TCI computation, whose output also feeds the unrelated Context Meter/Glass-Box Timeline telemetry. Not built — a real backend change with a genuine failure mode (a stale cache after a mid-session preset edit could wrongly suppress a confirmation in a mode chosen specifically for its confirmations), correctly out of scope for the frontend-only pass that shipped the "Use for all" shortcut.
