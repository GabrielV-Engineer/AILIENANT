...
| DEBT-106 | ~~Dispatch dev roles resolve `EDIT_EXECUTE_RBW` but run tool-less~~ **RESOLVED (12.7)** — the 8 developer roles resolve through `select_tools`/`resolve_tools`, the same substrate the agentic cell and coder grounding pre-pass use; no new arsenal builder | MEDIUM | Feature gap | 12.7 | RESOLVED |
| DEBT-107 | Autonomous LLM-driven `DispatchPlan` emission is deferred — `brain/dispatch_emitter.py` ships the full mechanism (injected `dispatch_plan_fn` hook + `AILIENANT_DISPATCH_DEBUG` synthetic + counter resets + graph wiring), but the production model prompt that decides when/how to fan out is not built; emission fires only via seam/hook or a directly-seeded `dispatch_plan` | MEDIUM | Feature gap | future dispatch-emission slice | Floating |
| DEBT-108 | `tests/benchmark/test_retention.py::test_run_benchmark_bounds_artifacts` is load-timing flaky — under machine load the `benchmark_service._persist_with_retention` cross-process `FileLock` intermittently times out (30s), degrading to durability-first "write without prune" so 5 runs leave 5 artifacts instead of the cap of 3. Reproduced ~1-in-3 both within the isolated `tests/benchmark/` group and in the full suite; passes 3/3 when the single test runs alone. Not a product defect (the degrade path is intentional) — a test that asserts the happy path without forcing lock quiescence. Fix: make the test acquire/assert under a guaranteed-free lock (e.g. reset `_retention_async_lock` + remove any stale `.retention.lock` in the `iso_bench` fixture, or assert prune-or-degrade rather than strict cap) | LOW | Test isolation / flakiness | future test-hardening slice | Floating |
| DEBT-110 | The web dashboard BYOM panel styles reference `--vscode-*` host variables (which never resolve in the browser SPA — the dashboard is HTTP-served, not a webview) and legacy `--color-*` names (never defined); several declarations lacked hex fallbacks and rendered transparent/broken surfaces. 11.0 added a transitional compatibility shim in `shared/theme.css` aliasing these names to real design tokens so BYOM renders correctly now. Enterprise target: migrate `dashboard.css` BYOM block + `BYOMPanel.tsx` off `--vscode-*`/`--color-*`/hardcoded hex onto the design tokens, then delete the shim | LOW | UI / design-token debt | Phase 11.4 | Floating |
| DEBT-109 | Context-utilization telemetry is emitted as flat pipe-delimited text (`core/telemetry_log.py::_emit` → `CATEGORY \| k=v \| ...`), not typed JSONL, so `core/benchmark/context_telemetry_report.py` must string-parse a brittle format (defensive `(ValueError, KeyError)` line-skipping is the mitigation, not a fix; a `_LINE_CAP`-truncated line silently drops that record). Accepted short-term trade-off — the sink shipped this format for all categories (WS/NODE/INDEX/CONTEXT). Enterprise target: migrate the sink + all consumers to a typed JSONL data contract (one JSON object per line) so telemetry has a schema, not a convention | LOW | Observability / data contract | future telemetry-format slice | Floating |
| DEBT-111 | GraphRAG nebula encodes only the two node types the file-level substrate exposes (`file`, `external-dep`); a containment-only `function`/`class`/`module` taxonomy from the already-populated `symbol_definitions` catalog is **not** blocked (it never enters `MAX_GRAPH_EDGES` accounting) — only a full call-edge-materialized graph would conflict with `SCHEMA_EVOLUTION.MD` "Symbol-Level Call-Graph Substrate". See the detailed entry below | MEDIUM | Capability gap / Graph | Phase 11.2.S | Floating |
| DEBT-112 | `ui/ConfirmModal` was extracted from BYOM's inline confirm modal but `BYOMPanel.tsx` still uses its own `byom-confirm-*` markup/CSS — migrate it onto the shared primitive and delete the panel-local styles | LOW | DRY / FE Architecture | Phase 11.4 | Floating |
| DEBT-113 | Nebula picking uses `raycaster.intersectObject` and the force layout runs one-shot on the main thread — both are fine at current node counts but the 100k-node target needs GPU-picking (render-to-id-texture) + a Web-Worker layout; the seams are in place (`nebula/picking` split, `nebula/engine` layout call) but the scale implementations are not built | LOW | Performance / scale | future nebula-scale slice | Floating |
| DEBT-114 | The nebula search pulse illuminates matched nodes + their incident edges, not the actual path GraphRAG traversed to answer a query — no retrieval trace is captured or emitted anywhere. A true reasoning-path replay needs the retrieval pipeline to record and surface the traversed node/edge sequence | LOW | Capability gap | future retrieval-trace slice | Floating |
| DEBT-115 | Per-project token-cost bucketing deferred from 11.1 — `token_ledger.snapshot()` (`main.py`, exposed via `/api/v1/telemetry/tokens`) is a process-global in-memory aggregate with no `project_id` dimension, so the dashboard's cost cards stay global and are honestly badged as such. A per-project view needs the FinOps ledger to bucket accrual by project (ephemeral across restart unless also persisted) | LOW | Feature gap | future FinOps slice | Floating |
| DEBT-116 | ~~Per-request latency (P50/P95) not captured~~ **RESOLVED (11.3.B.1)** — additive `request_latency` telemetry table, `_run_coding_task` `finally` emit, hand-rolled percentile reader, `/api/v1/telemetry/latency`, live TelemetryPanel card | MEDIUM | Observability gap | Phase 11.3.B.1 | RESOLVED |
| DEBT-117 | ~~Docker container lifecycle history not persisted~~ **RESOLVED (11.3.B.2)** — machine-global `container_lifecycle` table, started/stopped emitted from `core/sandbox.py` async wrappers, `/api/v1/runtime/lifecycle`, RuntimePanel span timeline | LOW | Observability gap | Phase 11.3.B.2 | RESOLVED |
...
| DEBT-153 | `core/response_cache.py`'s module-level `response_cache` singleton has no per-test reset fixture — two test files that happen to build an identical cache key (same `target_file`/context/project_id/model) can cross-contaminate when run in the same pytest process, one silently skipping generation via a cache hit from the other. Found pre-existing (confirmed via `git stash`, unrelated to the diff being tested) while adding new coder-path tests in 12.7 | LOW | Test isolation / flakiness | future test-hardening slice | Floating |
...
### DEBT-110 [LOW · Floating] — BYOM panel styles reference unresolved host/legacy CSS variables

- **Date:** 2026-07-20
- **Reproduce:** open the served web dashboard (a browser SPA, not a VS Code webview) and inspect the BYOM panel. The `.byom-*` rules in `ailienant-extension/src/dashboard/dashboard.css` reference `--vscode-editor-background`, `--vscode-sideBar-background`, `--vscode-panel-border`, `--vscode-descriptionForeground` (host webview variables, absent in a plain browser) and `--color-primary`/`--color-card`/`--color-text`/`--color-border`/`--color-primary-soft` (never defined anywhere). Declarations with an inline hex fallback (`var(--x, #hex)`) survived; those without one (e.g. `.byom-confirm-modal { background: var(--vscode-editor-background) }`) rendered transparent/broken.
- **Error:** UI/design-token debt — cosmetic, not a functional defect.
- **Mitigation (11.0):** `shared/theme.css` gained a transitional compatibility shim aliasing each referenced name to a real design token, so BYOM renders correctly immediately.
...
### DEBT-153 [LOW · Floating] — `response_cache` singleton has no per-test isolation, causing order-dependent cross-file cache hits

- **Date:** 2026-08-03
- **Reproduce:** `python -m pytest tests/test_coder_agent.py tests/test_tool_dispatch.py -q` — `test_tool_dispatch.py::test_coder_injects_active_skill_directive` fails (asserts the injected skill directive appears in the captured system prompt; it doesn't, because generation never ran). Passes when run alone or as part of the full suite's default collection order; only reproduces under this specific two-file ordering. Confirmed pre-existing on `main` via `git stash` before this diff's changes — not a regression introduced by 12.7.
- **File(s):** `ailienant-core/core/response_cache.py:163` (`response_cache = SemanticResponseCache()`, process-global, no test-scoped reset); `ailienant-core/tests/conftest.py` (has autouse resets for the skill-embedding cache and sandbox pool state, but none for this cache).
- **Error:** test-isolation gap. An earlier test file (`test_coder_agent.py`) exercises `run_coder_node` against a step whose `(target_file, current_content, rag_snippets, budget)` tuple happens to build the identical cache key a later test in `test_tool_dispatch.py` also builds; `response_cache.probe(cache_key)` returns the earlier test's stored (empty) generation, so the later test's `LLMGateway.acomplete_with_thinking` mock is never invoked and its capture-based assertion sees no messages at all.
- **Blocked by:** none.
- **Phase:** future test-hardening slice — add an autouse `response_cache.clear()`-equivalent fixture in `tests/conftest.py`, mirroring the existing `_reset_skill_embed_cache` fixture's pattern.
- **Notes:** not a production defect — the cache's real-world behavior (identical inputs return the identical prior output) is correct by design; only the test suite's shared-process assumption is at risk. Worked around in 12.7's own new tests by keeping `target_file` values unique per test rather than fixing the shared fixture, to keep that slice's diff scoped to DEBT-130/129/106/127.
