...
| DEBT-116 | Per-request latency (P50/P95) is not captured anywhere in the engine/gateway, so 11.3's `TelemetryPanel` shows a "coming in 11.3.B" affordance instead of a latency chart. Needs request-timing instrumentation on the graph/gateway request path + a percentile rollup | MEDIUM | Observability gap | Phase 11.3.B | Floating |
| DEBT-117 | Docker container lifecycle history is not persisted — `/api/v1/runtime/status` returns only current flags, so 11.3's `RuntimePanel` can show a live tier ladder but not a start/stop Gantt. Needs a timestamped container lifecycle event log | LOW | Observability gap | Phase 11.3.B | Floating |
| DEBT-118 | No adapter set-tier endpoint — the execution tier is auto-resolved and 11.3's tier ladder is read-only. A manual tier switcher needs a backend set-tier control with validation + persistence | LOW | Feature gap | Phase 11.3.B | Floating |
| DEBT-119 | No live container log stream — `RuntimePanel` cannot tail sandbox/container logs; needs a bounded streaming log endpoint (WS or chunked) with backpressure | LOW | Feature gap | Phase 11.3.B | Floating |
