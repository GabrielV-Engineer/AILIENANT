...
| DEBT-075 | Syntactic-only symbol extraction; no LSP-style type resolution | LOW | Capability gap | long-term | Unscheduled |
...
| DEBT-025 | Docker PTY no daemon integration test | LOW | Test coverage | future CI-daemon-lane slice | Blocked |
...
| DEBT-180 | `agents/analyst.py::_is_agreement` does substring matching against short tokens ("ok", "yes", "bien") — a substantive answer that happens to contain one ends the Socratic grill early, mistaking elaboration for a bare agreement (blast radius narrowed by 13.0.5: now a first-turn fast path only) | LOW | Correctness gap | future ideation-quality slice | Floating |
| DEBT-183 | The Coder Companion (`brain/coder_companion.py`) explains only coding turns, once, after the fact — patch-shaped input contract, single terminal WS blob, frontend hard-gated on `diffBlocks.length > 0`. User wants full step-by-step transparency (what + why the system is doing, incl. Plan-mode investigation) across every mode. Scoping brief: `docs/COMPANION_TRANSPARENCY_BRIEF.md` | MEDIUM | Feature gap / UX | design session with the user before any implementation | Floating |
...
- **Date:** 2026-08-17 · **Narrowed:** 2026-08-17 (13.0.1)
- **Reproduce:** the ambiguity pre-flight gate and the HIGH-risk Plan-mode suggestion (`agents/researcher.py`, `core.hitl.request_graph_clarification`) put a `suggested_options: List[str]` list on the `interrupt()` payload, but the frontend's existing HITL approval-card component only renders a binary approve/reject affordance (the same one `drift_gate` uses). `suggested_options` currently reaches the WebView unrendered — the user answers via the card's free-text comment box, not a real multi-select.
- **Narrowed by 13.0.1 (closing DEBT-171):** originally this entry also covered a channel that was inert in *both* directions — the outbound card didn't even show the question text, and the inbound answer was discarded regardless of what the user typed. Both of those are now fixed additively (`api/ws_contracts.py`'s `question`/`context`/`suggested_options`/`answer`/`selected_option` fields, `task_service.py::_emit_interrupt_card`, `main.py::_resume_approval_dict`) — the channel round-trips correctly today through the existing plain approve/reject card. What remains is purely the visual multi-choice affordance.
- **File(s):** `core/hitl.py` (`request_graph_clarification`), `api/ws_contracts.py` (the additive fields, already present), `ailienant-extension/src/workspace/` (the HITL approval-card component — needs a `suggested_options`-aware render branch that calls `onRespond` with `selected_option` instead of relying on the comment-box fallback).
- **Error:** UX gap only, explicit MVP declaration per CLAUDE.md §11.2 — the backend contract is additive and forward-compatible (an old card renderer just ignores the extra fields), and correctness no longer depends on this landing.
- **Blocked by:** frontend work.
- **Phase:** future HITL-card-UX slice.
- **Notes:** logged at ship per CLAUDE.md §11.3, alongside DEBT-168/169/170/171 (2026-08-17 harness-audit sweep); narrowed when 13.0.1 closed DEBT-171's backend half.
...
### DEBT-183 [MEDIUM · Floating] — Coder Companion explains only coding turns, once, after the fact

- **Date:** 2026-08-19
- **Was/is:** `brain/coder_companion.py::_run_coder_companion` has a single call site (`agents/coder.py:984-987`), a patch-shaped input contract (`_build_companion_request` reads `pending_patches`/`pending_contents`), and emits one terminal WS blob (`CoderCompanionPayload`) that `workspaceStore.ts` replaces by `task_id` — no streaming/append semantics. `Workspace.tsx:698-700` hard-gates the card on `diffBlocks.length > 0`, so it structurally cannot render outside a coding turn with a diff.
- **User's ask:** full step-by-step visibility into what the system is doing and *why*, in every mode — including how Plan mode is investigating, what errors a coding turn hit and how it corrected them, how decisions get made — surfaced incrementally in chat, not as one summary at the end.
- **Not a small fix:** the Glass-Box Timeline (`server_activity_event` → `AgentTimeline.tsx`) already covers the mode-agnostic, step-by-step *what*, un-throttled, since 13.0.6 including the Socratic grill's own two phases. The gap is the *why*, which only exists for code-writing turns today. Generalizing the companion needs a new input contract per mode (or one contract broad enough for all three), a decision on per-step vs. per-round granularity, and the same cost/latency governance the current single call already needs (budget, VRAM slot, 3-way semaphore, timeout) scaled to however many calls the new granularity implies.
- **File(s):** `brain/coder_companion.py`, `api/ws_contracts.py` (`CoderCompanionPayload`), `ailienant-extension/src/workspace/components/CoderCompanionCard.tsx`, `ailienant-extension/src/workspace/workspaceStore.ts`, `ailienant-extension/src/workspace/Workspace.tsx:698-700`.
- **Phase:** design session with the user before any implementation — brief at `docs/COMPANION_TRANSPARENCY_BRIEF.md` frames the open questions (input contract, granularity, division of labour with the Timeline, cost governance, frontend delivery).
- **Notes:** logged rather than implemented per explicit instruction this session — the four other items in this batch (ruff, the dropped-answers bridge bug, the replay-invariant violation, the invisible-turn UX gap) were fixed; this one is scoped only.
