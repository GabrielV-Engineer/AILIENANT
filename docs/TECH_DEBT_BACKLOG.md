...
| DEBT-106 | Dispatch dev roles resolve `EDIT_EXECUTE_RBW` but run tool-less — no static `RegisteredTool` arsenal builder exists for the 8 developer roles (only read-only analyst/researcher builders + runtime MCP registration); the per-role permission machinery is fully wired, the executable tooling is not | MEDIUM | Feature gap | future dev-arsenal slice | Floating |
...
| DEBT-119 | No useful container log surface — the dashboard is HTTP-only (no WS/SSE) and the cage runs `tail -f /dev/null` (empty stdout; commands exec per-call). Reframed: capture per-`exec_run` stdout/stderr into a bounded per-session ring, polled via `GET /api/v1/runtime/exec-log?tail=N` | LOW | Feature gap | Phase 11.3.B.3 | Floating |
| DEBT-120 | Append-only growth of the new telemetry tables — `request_latency` (one row per task) and `container_lifecycle` grow unbounded; reads are windowed/clamped but the tables are not pruned. `core/janitor.py` exists; wire retention/GC of old rows into it (both tables carry `# TODO(retention): DEBT-120` markers) | LOW | Data retention | future janitor slice | Floating |
