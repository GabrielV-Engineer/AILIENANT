...
| DEBT-111 | GraphRAG nebula encodes only the two node types the file-level substrate exposes (`file`, `external-dep`); `function`/`class`/`module` shapes need the `symbol_definitions`/`observed_call_edges` tables that `SCHEMA_EVOLUTION.MD` "Symbol-Level Call-Graph Substrate" deliberately keeps out of the analytics graph (would exceed `MAX_GRAPH_EDGES=5000`). Requires amending that frozen decision + a symbol-edge analytics pipeline before the taxonomy can grow | MEDIUM | Capability gap / Graph | Phase 11.2.S | Blocked |
