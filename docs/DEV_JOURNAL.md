...
## 13.0: OpenSpec Verification-Gate Integration — 2026-08-17
**Status:** COMPLETE | **Gates:** `openspec validate --all --strict` 1/1 passed · docs/CI-only, no mypy/pytest/pyright surface touched
- Shipped: installed `@fission-ai/openspec` as an ADDED, ADVISORY spec-drift verification layer (root `package.json`, decoupled from `ailienant-extension`'s packaging) — `docs/PROJECT_MANIFEST.md`/`DEV_JOURNAL.md`/`SCHEMA_EVOLUTION.MD`/`TECH_DEBT_BACKLOG.md` remain the source of truth, unreplaced. Wired `openspec init --tools claude`, `.github/workflows/openspec-gate.yml` (`continue-on-error: true`), `CLAUDE.md` §15 scope-boundary section, and authored the Phase 13 pilot (`openspec/changes/13-portfolio-level-release/`) converting `13.1`-`13.5` into requirement/scenario form in lieu of a traditional blueprint.
- Key decision: install location is a dedicated root `package.json`, not `ailienant-extension/package.json` (DEBT-167) — keeps a repo-wide governance tool's dependency graph off the extension's esbuild/vsce pipeline. `AGENTS.md`, found stale (still referencing the long-closed Phase 7.16/7.17 lock-in), was reduced to a thin pointer at `CLAUDE.md` before `openspec init` ran, rather than re-synced in full, so it can't drift into a second charter copy again.
- Deferred: DEBT-165 (new-phases-only scope, no Phase 0-12 backfill), DEBT-166 (advisory-to-blocking promotion criteria not yet decided).

## 13.0.1: Post-Audit HIGH-Debt Closure — 2026-08-17
**Status:** COMPLETE | **Gates:** mypy 0/467 · pytest 2951 passed, 2 skipped · pyright 0 · npm compile 0 · npm test 191 passing
- Shipped: closed DEBT-168 (Vision Bypass override + `LLMGateway.ainvoke` multimodal `images=` payload), DEBT-170 (`is_session_busy` admission guard + abandoned-pause TTL reclamation + `PromptBar` `isStreaming` gate), DEBT-171 (defer-then-interrupt-first two-phase suspend for `ask_user_question`, mirroring the DEBT-129 `pending_tool_call` shape).
- Key decision: DEBT-168's own backlog entry and DEBT-171's literal fix prescription were both wrong — `RoutingEngine.resolve_provider` has zero production callers (the routing half was never wired, not "already shipped"), and `interrupt()` cannot run mid-tool-dispatch-loop, so DEBT-171 needed the two-phase defer pattern rather than a direct repoint. Closing DEBT-171 also surfaced and fixed two unnamed bugs: the outbound HITL card dropped `question`/`context`/`suggested_options` and had no `request_id` correlation fallback, and the inbound resume value was hardcoded to `{approved, comment}` — the same-morning ambiguity gate was inert in both directions until this fix (`SCHEMA_EVOLUTION.MD §51`).
- Deferred: DEBT-174 (coder-node multimodal payload, new), DEBT-172 (FE multi-choice renderer, narrowed — the channel itself now round-trips correctly).

## 13.0.2: Adaptive Tool Selection — 2026-08-18
**Status:** COMPLETE | **Gates:** mypy 0/468 · pytest 2970 passed, 2 skipped · pyright 0 · npm compile 0 · eslint 0
- Shipped: routed the agentic cell, swarm, and subagent-worker tool selection through `DeferredToolLoader` sized by `resolve_context_budget` — so a fitting role slice is injected whole and `tool_search` always survives an over-cap cut; added a per-iteration composed retrieval intent (`_compose_cell_intent`, pure) plus the `cell_tool_query` replay channel (`SCHEMA_EVOLUTION.MD §52`); advertised only `filter_resolvable` output so the advertised set equals the dispatchable set; and denied `toggle_plan_mode` in the cell, since whole-catalog injection would newly expose the self-escalation lever over the channel `evaluate_action` gates.
- Key decision: three defects were structural, not cosmetic — the eager branch had NEVER fired in production anywhere (`swarms.py` read `context_window`, a field of `LLMProfile` and never a graph channel, so it always fell back to 8192, below every role's slice); cross-listed tools were role-NARROWED by a name-keyed upsert AND again by `resolve_tools`'s `dict.update`, so fixing one layer alone still denied the analyst at dispatch; and `tool_search`'s "name it next turn" instruction was unfulfillable in a node re-entered with a frozen intent. `TOOL_RAG_TOP_K` stays 5: raising it fails the Phase-5.7 gate, whose prescribed description-compression remedy is near self-cancelling (descriptions are 23.3% of payload, and compressing shrinks both sides of the ratio) — callers needing N usable tools pass `k=N+1` instead.
- Deferred: DEBT-175 (k ceiling vs the gate's subset baseline), DEBT-176 (tool-invocation telemetry; the ranking prior rejected outright on `select_tools` determinism), DEBT-177 (three declared conservatisms).

## 13.0.3: Loop-Unsafe Tool Exclusion — 2026-08-18
**Status:** COMPLETE | **Gates:** mypy 0/468 · pytest 2974 passed, 2 skipped · pyright 0 · npm compile 0 · eslint 0
- Shipped: 13.0.2's eager wiring left `toggle_plan_mode` reachable from `brain/nodes/subagent_worker_node.py`'s dispatcher deterministically (the cell's denylist did not cover it) and from `agents/coder.py`'s grounding pre-pass (a pre-existing, independent exposure — its READ_ONLY filter passes it, and `select_tools`'s READ_ONLY-survivor guarantee actively promotes it when the ranking has no other candidate). Replaced the module-local cell denylist with `core/tool_registry.py::filter_loop_safe` (backed by a named `_NO_AUTONOMOUS_LOOP` dict), applied at all three dispatch-loop consumers plus a second, previously-unguarded exact-name resolve site in the cell (the pending-tool-call approval resume).
- Key decision: severity is contained, not privilege escalation — corrects an earlier overstatement. `core/tool_dispatch.py::_STATE_PROMOTERS` allowlists only `todo_write`, so the tool's write never reaches a returned delta; `brain/dispatch.py`'s per-`Send` state copy prevents cross-subagent leakage; `ToolDispatcher` pins `session_mode` at construction. The real defect is a "tool that lies" one layer over `filter_resolvable`: `toggle_plan_mode` is deliberately READ_ONLY-tier ("policy-neutral across the matrix"), so the privilege system cannot gate it — a reasoning loop can call it, get a success observation, and nothing happens. Kept as a separate constant from `_INTENTIONALLY_UNREGISTERED` rather than merged: that dict means "no factory / no reachable role," both false here, and folding it in would have broken either the advertised==dispatchable invariant or the direct-resolution contract `tests/test_tool_registry.py` pins.
- Deferred: DEBT-178 (the tier system has no vocabulary for "mutates the policy engine itself," so every future dispatch-loop consumer must remember this filter).

## 13.0.4: Multi-Question Clarification Grill — 2026-08-18
**Status:** COMPLETE | **Gates:** mypy 0/468 · pytest 2979 passed, 2 skipped · pyright 0 · npm compile 0 · eslint 0 · npm test 198 passed
- Shipped: closed DEBT-172. The clarification wire payload carried `question`/`context`/`suggested_options` since 13.0.1, but `contracts.ts` never declared those fields (silently dropped) and had no options renderer, so every clarification degraded to the plain free-text approve/reject card; `AskUserQuestionTool`'s schema also gave the LLM no structured way to express more than one concrete option. Added an additive `questions`/`answers` batch extension end-to-end (`SCHEMA_EVOLUTION.MD §52`) and a new `ClarificationGrillCard.tsx` (tab strip + options panel, one "(Recommended)" option, free-text "Other" row), normalizing the legacy single-question shape into a one-item batch so the ambiguity-gate and Plan-mode-suggestion callers get the options UI for free with zero changes to those two call sites.
- Key decision: kept `AskUserQuestionInput`'s legacy singular fields (`question`/`context`/`suggested_options`) fully intact rather than replacing them with the batch shape — the LLM-facing schema could have been redesigned cleanly since the two internal callers bypass the tool entirely, but the additive path costs nothing and avoids any risk to already-shipped single-question call sites.
- Deferred: none — DEBT-172 fully closed, not narrowed.

## 13.0.5: Grill Me Batched Questions + Tool Field-Description Delivery — 2026-08-18
**Status:** COMPLETE | **Gates:** mypy 0/469 · pyright 0 · pytest green · npm compile 0 · eslint 0
- Shipped: 13.0.4's card was correct but never appeared in live Plan-mode testing — two independent causes. (1) `core/tool_dispatch.py::build_schema_hint` rendered only each tool's first docstring line plus bare argument NAMES, silently discarding every `Field(description=...)` and never expanding nested `$ref`'d models — so 13.0.4's option-generation guidance never reached the model at all; now renders per-field descriptions recursively. (2) Plan mode never used `ask_user_question`: it is the Socratic Grill Me ideation subgraph, which predates the clarification channel and streamed one free-text question per turn. Converted to batch 2-6 structured questions per round through the same `request_graph_clarification`/`ClarificationGrillCard` channel, with an empty batch as the model's own completion signal.
- Key decision: the per-round pause moved *inside* `analyst_grill` (native `interrupt()`), so `route_after_analyst` now self-loops instead of returning END, and the round counter had to become a declared state channel (`grill_round_count`, `SCHEMA_EVOLUTION.MD §53`) — each round is its own super-step with a fresh Python frame, so a local counter cannot survive between rounds. `task_service.py` needed no change on the pause side: `extract_pending_interrupt` is already checked before the legacy `hitl_pending` branch. Declared tradeoff (§11.2): the batch no longer streams token-by-token, since a structured JSON call is inherently non-streaming — matching how the planner's own structured generation already works.
- Two defects the full-suite run caught and fixed before close: swapping `astream_byom` for `ainvoke("ailienant/medium")` silently changed the BYOM-down contract — the alias falls through to the litellm proxy when no preset is active, so a dead grill burned the full transport-retry budget (310s in one test) instead of failing fast; `_generate_grill_questions_llm` now resolves `get_chat_target` up front and returns `None` (distinct from an empty batch, the model's real completion signal), and the node surfaces the actionable BYOM notice with `hitl_pending=True`. That in turn required `route_after_analyst` to check `hitl_pending` FIRST and return END — without it the new self-loop would retry an unreachable model to the recursion limit. `tests/test_engine_respine.py`'s ideation case also documented an analyst stub it never actually enabled (`DEBUG_MODE` is read into a module constant at import, so the env var alone was inert), which is why it reached the network at all; it now patches the resolved attribute and asserts the real card path end-to-end.
- Deferred: none. DEBT-180's blast radius narrowed (agreement-phrase matching is now a first-turn fast path only, since the model's empty-batch response is the primary completion signal).

## 13.0.6: Grill Answer-Loss Fix + Turn-Wide Busy Indicator — 2026-08-19
**Status:** COMPLETE | **Gates:** ruff 0 (newly gated) · mypy 0 · pyright 0 · pytest green · npm compile 0 · eslint 0 · npm test 208 passed
- Shipped: closed two real bugs live testing surfaced past 13.0.5. DEBT-184 — the extension-host `HITL_RESPONSE` bridge (`providers/workspace_panel.ts`) forwarded only the four original approve/reject fields; `answer`/`selected_option`/`answers` were silently dropped for a full release, discarding every clarification answer despite every other link in the chain round-tripping correctly in isolation — fixed by adding the fields and extracting a testable `buildHitlResponseData`. DEBT-185 — `run_analyst_node` generated its question batch and called `interrupt()` in the same invocation, so a LangGraph resume replay could regenerate a *different* batch than the one answered; split into a generate phase (commits `pending_grill_batch`, no interrupt) and an ask phase (resolves the state-sourced batch as its first action) — the exact defer-then-interrupt-first shape `brain/agentic_cell.py` already uses for this reason. Also added grill narration (`grill_grounding`/`grill_composing_questions`) and a new `isTurnActive` chatStore flag — wider than `isStreaming` (token delivery only) — gating the Stop button, Esc, submit-blocking, and the Active Task Header (now showing real `AgentTimeline`-sourced narration instead of a literal "Working…"), set at submit and at every HITL/clarification reply.
- Key decision: the two bugs compounded into what read as "the interview hangs forever" — the dropped answers meant the grill was actually waiting correctly, but with no narration and a token-only busy indicator there was no visible sign of that, and no way to cancel. Fixing all three (answers, replay-safety, visibility) together was necessary; any one alone would have left the symptom.
- Deferred: DEBT-183 (generalizing the Coder Companion beyond coding turns to full step-by-step transparency across every mode) — scoped to a design brief (`docs/COMPANION_TRANSPARENCY_BRIEF.md`) per explicit instruction, not implemented this session.

## 13.1: Full Dockerization — 2026-08-18
**Status:** COMPLETE | **Gates:** docker compose build 0 · docker compose up → healthy (verified end-to-end, port 8000) · openspec validate 13-portfolio-level-release --strict 1/1 passed
- Shipped: root-level `Dockerfile` (app-runtime image, distinct from the pre-existing sandbox `ailienant-core/Dockerfile`) + `docker-compose.yml` + `.dockerignore`; `docker compose up` from a clean checkout reaches a healthy backend with zero manual steps, embedded LanceDB/SQLite persisted to a named volume.
- Key decision: `uvicorn` is invoked directly with `--host 0.0.0.0` in `CMD` rather than via `python main.py` (whose `__main__` block hardcodes `127.0.0.1` for the extension's local-spawn path) — binding loopback-only inside the container would make the published port unreachable despite the compose port mapping. Also found and fixed a real Docker gotcha: mounting a fresh named volume over `/home/ailienant/.ailienant` (a path with no matching content in the image) initializes the mount point owned by root, blocking the non-root runtime user's SQLite catalog init — fixed by pre-creating that directory with correct ownership before the volume mounts over it.
- Deferred: DEBT-179 (the image installs the full `requirements.txt`, including dev/test-only tooling, unsplit from runtime deps).

## Fix: Ideation→planner handoff permanently dark (`ideation_synthesized` never declared) — 2026-08-18
**Status:** COMPLETE | **Gates:** mypy 0/469 · pytest 2988 passed, 2 skipped · pyright 0
- Shipped: the Plan-Mode Socratic grill could never reach the planner — `synthesis_node` signaled the handoff via `ideation_synthesized`/`ideation_glossary`, neither ever declared on `AIlienantGraphState`, so LangGraph's compiled graph silently dropped both on every write (no exception, no warning) and `route_after_ideation` always fell to its defensive `ideation_no_op` branch, surfacing as the generic "the planner did not return a plan". Declared both channels; added a runtime `assert_declared_channels` guard (raises under pytest/`AILIENANT_STRICT_STATE=1`, else logs) wired into every parent-graph node and the ideation subgraph's own nodes (which bypassed `_instrument_node` entirely — the gap that hid this); made the `ideation_no_op` dead-end name itself in the user-facing message instead of impersonating a planner failure. Also fixed DEBT-181 in the same pass (user instruction): the grill's two history-replay sites dropped the summarizer's compacted `[HISTORY SUMMARY]` entry, silently forgetting a long dialogue's earlier turns.
- Key decision: the guard is runtime and node-scoped, not a static AST scan — a scan run for comparison flagged ~25 files of ordinary helper-function returns as false positives, so only the actual delta a node returns at the LangGraph write boundary is checked.
- Deferred: DEBT-180 (grill's substring-match agreement detector can end a turn early on a substantive answer that happens to contain an agreement token).

## 13.0.7: Work-Loop Transparency — Real Reasoning Inline, Companion at Decision Points — 2026-08-20
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/469 · pyright 0 · pytest green · npm compile 0 · eslint 0 · npm test 236 passed
- Shipped: closed DEBT-183. Rendered every `reasoning` timeline entry independently (was: only the first per turn, from message-scoped props) with its own per-entry chronometry and toggle — the primary model's own reasoning now IS the step-by-step trace, at zero new LLM cost. Retired the Companion's post-hoc "simulated" narration pass (a second model dubbing the first). Generalized the Companion beyond patches: new `scope`/`scope_summary` on its request, three scope builders each consuming only their own decision point's slice, firing at a grill round closing, a plan committing, a patch landing, and error correction resolving — governed by the existing semaphore/budget/VRAM guards plus a new shared per-task emission cap. Frontend storage moved from a session-wide last-write-wins store to message-scoped append storage (`Message.companions`), ungating the card from `diffBlocks.length > 0`; `AgentTimeline` gained a purely-frontend work-loop phase grouping (gather/act/verify) over existing rows; `PlanAcceptancePanel` switched from `isStreaming` to `isTurnActive` (same bug class fixed for other surfaces in 13.0.6).
- Key decision: rejected the initial per-phase-companion-emission draft on audit — it rested on the now-corrected reasoning-latch premise and had no backend phase counterpart to bound it. The shipped design fires the Companion at real graph decision points instead, each already topology-bounded (`_GRILL_MAX_ROUNDS`, `CORRECTION_MAX_ATTEMPTS`), and keeps the gather/act/verify axis purely frontend with no new wire field, disambiguating `command` rows via a small additive `metric` value rather than inventing a phase channel.
- Deferred: DEBT-183 closed; logged the `CoderCompanion*`→`AgentCompanion*` rename (no functional gain this batch, needs a dual-emit window) and noted `emission_id` collision risk on a mid-task replan (planning-scope `attempt_ordinal` stays 0, no replan counter exists yet).

## 13.0.8: HITL Resume Session Routing + Active-Task State Restoration + Local-Timeout Scaling — 2026-08-20
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/471 · pyright 0 · pytest 3031 passed/2 skipped · npm compile 0 · eslint 0 · npm test 242 passed
- Shipped: closed DEBT-188 (the reported "stuck for hours" after answering a grill question), DEBT-189 (active-task state loss), DEBT-190 (ideation Companion always degraded on local hardware), and DEBT-191 (the planner failing every attempt on slow local hardware, "6 validation errors ... input_value={}") — all found live in the same session, none a 13.0.7 regression. 188: `main.py` resumed a paused graph by the WS connection's own `client_id` instead of the chat's `session_id`, silently missing whenever more than one session shared a connection; fixed by adding `session_id` to `HITLResponsePayload`. 189: the active-task header/spinner vanished on any tab switch (pre-existing since Phase 11.6, `retainContextWhenHidden:false`); mirrored the transcript-rehydrate pattern with a new `ACTIVE_TASK_RESTORED` message. 190: the ideation Companion raced the grill's own next local LLM call for the same Ollama server (measured: solo call ~11.4s, two concurrent 28.4s total); fixed with a pre-check that skips cleanly before attempting. 191: the flat 300s local-model timeout couldn't fit a large structured-generation call on hardware measured at ~2-3 tokens/sec; replaced with `resolve_local_timeout(max_tokens, model)` — static formula seed, refined by per-model adaptive calibration (duration-weighted, gated on sample count AND accumulated tokens after a live-found bug where two tiny samples produced a ~3.6-hour estimate). A second live-testing round on this same shipped UX then found three more gaps, all closed in this same batch: (A) a PLAN_ONLY turn had no graph-level pause (`planner_agent → drift_compute` was a static edge), so the plan visibly ran to completion (RBAC-denied but fully narrated) before the Accept panel's buttons ever enabled — fixed with a new `route_after_planner` conditional edge to `END`, plus an always-enabled panel close button; (B) repeated tab-switching progressively destroyed state via a debounce-vs-teardown race, a `mergeById` that deleted rather than reverted unpersisted completed messages, a reveal-time listener race, and a structural gap hiding the WBS checklist behind an emptied timeline — fixed with immediate-flush persistence, a broadened merge, a `WEBVIEW_READY` handshake, and decoupling the checklist from `AgentTimeline`'s entries gate; (C) the Companion's coding-scope path still degraded every WBS step on slow hardware because its outer `asyncio.wait_for` kept a flat 45s deadline even after 191 scaled the inner call — migrated onto `resolve_local_timeout` directly.
- Key decision: the user pushed for two separate deeper-analysis passes on 191 specifically before accepting it as done — one audit of the live-coded diff itself (which caught a real test-collection regression, an unacknowledged retry-path timing bug, and zero pytest coverage for entirely new logic), one broader pass on the tradeoff the fix introduced (larger timeouts also mean a genuinely dead endpoint surfaces its error later; litellm/OpenAI-SDK source confirmed retries multiply that further for no benefit on a local timeout, and `agentic_cell.py`'s per-turn budget was implicitly self-consistent with the old flat timeout in a way the new scaled one broke). Both passes found real issues an ad-hoc live-verification pass had missed. The A/B/C round followed the same discipline: three Explore-agent audits plus a Plan-agent design pass, all reading the live tree directly, before any code was written.
- Deferred: DEBT-192 (calibration recorded only from `ainvoke`, not the direct-BYOM streaming paths — deliberate, confirmed with the user), DEBT-193 (a local timeout doesn't trigger failover, unlike a connection drop), DEBT-194 (no liveness signal exists to distinguish slow-but-working from dead — a proper fix needs `ainvoke` to move onto streaming), DEBT-195 (the benchmark harness's own separate flat sandbox timeouts, and one latent unscaled path off the primary route), DEBT-196 (HUD/telemetry has no rehydration message — a live reading, not lost data).

## 13.0.9: Incremental Approval, Honest Reporting, and a Visible Work Loop — 2026-08-25
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/474 · pyright 0 · pytest 3115 passed/2 skipped · npm compile 0 · eslint 0 · npm test 258 passed
- Shipped: replaced the post-graph whole-WBS approval replay with two graph nodes (`apply_patch` PREPARE, `apply_commit` interrupt-first GATE — new `brain/apply_gate.py`), closing three live bugs at once: a step now reaches `completed` only after its real decision is known (was: the instant the coder generated it); approving files 1-3 then requesting changes on file 4 can no longer discard the first three; a plain reject is a terminal `rejected` with a recorded error instead of silently vanishing. Closed a security gap where Ask-mode EXECUTE-tier commands fell through to a real shell spawn with no approval card, plus an unvalidated command string that once reached a real shell as `"N/A"`. Root-caused and fixed the actual reported UI bugs: the stuck approval panel and the tab-switch state-loss cascade both traced to `attachOrUpdateTimeline`'s placeholder never marking itself `streaming: true` (a first-frame auto-collapse AND a two-message split) and — the real culprit for the progressive checklist/plan/transcript loss — `useSessionPersistence.ts`'s fast-path `InflightSnapshot` never carrying `checklist`/`companions`/`timeline`, only prose, since those surfaces postdated its original design. Corrected the timeline's collapse policy (current turn stays open through its own completion, collapses only when a newer turn starts), removed its 340px internal cap, defaulted command rows open, and wrote the entire missing `ws-companion-*` CSS block (21 classes, previously zero). Wired the long-declared-but-never-emitted `retrieval` activity kind at two real call sites and added a new `subagent` kind so a dispatched subagent's work is no longer invisible. Moved the polyglot tool-directive off the human-facing WBS description onto a new agent-only field; gave the self-heal companion real command/exit-code/output context instead of three generic lines; extended the planner's existing separate-reasoning-pass pattern to the coder, closing the last case where the native-thinking toggle did nothing at all. Found and fixed, independent of the plan: a `main.py` lifespan bug where `vfs_manager.shutting_down` was never reset on startup, permanently rejecting every WS connection after a second app boot in one process.
- Key decision: a second, deeper self-audit pass (explicitly requested before implementation) caught two defects that would have silently destroyed the whole feature — `reflexion_guard`/`dead_letter_decorator` both catch `GraphInterrupt` (a subclass of `Exception`) and would have converted every future approval pause into a healing signal or a bogus DLQ row. Fixed with `except GraphBubbleUp: raise` ahead of both broad handlers, the same discipline the codebase already applies to `asyncio.CancelledError`. The three step-status selection predicates (dispatch, stall guard, advance) were unified behind shared `is_dispatchable`/`is_terminal` helpers rather than three separate literal tuples, closing the exact drift vector the pre-existing code's own comment had already flagged as a risk. Declined to add a "Native/Simulated" reasoning-provenance badge for the coder — a prior session removed that exact badge from the planner once real reasoning became visible either way, and the same reasoning applies here.
- Deferred: DEBT-197 (`depends_on` unenforced at dispatch — a step whose prerequisite was rejected still runs), DEBT-198 (`pre_patch` hooks now cost N× per turn instead of once, a necessary tradeoff of per-step gating), DEBT-199 (the apply gate assumes SWARM dispatch stays dormant — untested for concurrent same-file writes), DEBT-200 (no one-click revert; VS Code Local History is the only recovery path today — declared MVP, not an oversight), DEBT-201 (one e2e test flaked once across three full-suite verification runs, preceded in the log by an apparent litellm-mock leak from an unidentified earlier test; passed cleanly in isolation immediately after — suite-wide test-isolation hygiene, not a product defect). Also left open, by design: chronological checklist repositioning (the current pinned placement has a documented persistence-survival rationale) and a possible dedicated bottom-anchored working indicator (likely already subsumed by the uncapped-growth + existing outer auto-scroll, unverifiable without a live UI pass).

## 13.1.1: Technical-Debt Sweep to Sub-30 Before v1 — 2026-08-25
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/476 · pyright 0 · pytest 3138 passed/2 skipped (1 environment-dependent error, new DEBT-202, unrelated to this sweep's own changes) · npm compile 0 · eslint 0 · npm test 264 passed
- Shipped: reconciled `docs/TECH_DEBT_BACKLOG.md` before touching anything — the naive 67-open count was wrong on both axes (9 dashboard-row-only "ghost" entries with no body, 12 body entries with no dashboard row, 2 RESOLVED entries a strikethrough header hid from every count, one true duplicate), giving a corrected 77 open. Classified all 77: 39 CAPABILITY and 12 DECISION relocated to two new non-defect sections (content preserved verbatim), 2 ALREADY-FIXED (the DEBT-172 duplicate; DEBT-067, whose body already documented a shipped fix its header was never flipped for), 10 fixed with a regression test each (DEBT-073/146/147/162/163/173/180/196/197/201), 14 DEFER-WITH-REASON. Verifying DEBT-201's flake to close it surfaced a real, previously-misdiagnosed root cause — `TestClient(main.app)`-based fixtures autoconnect to the real, non-test-isolated MCP catalog DB — logged as new DEBT-202 rather than fixed mid-sweep. Net: 77 → 15 open, under the sub-30 target.
- Key decision: DEBT-163's fix uncovered a second unwrapped `docker.from_env()` call the entry's own text had misdescribed as already safe; DEBT-180's first anchored-matching draft broke the frontend's own canonical `"Looks good, proceed."` plan-acceptance phrase, caught by re-running the full suite before considering it done, not by the file-scoped tests alone — both corrected in the same pass rather than shipped as known-partial.
- Deferred: DEBT-120/164/179 (originally scoped as fixes, moved to DEFER on a risk-adjusted call — real regression surface on a LOW-tier item, not worth the blast radius immediately pre-release), DEBT-161 (re-measured at ~419 refs/~174 files, was 471/130 — a ~174-file comment-only scrub is pure blast radius for zero runtime benefit this close to v1), DEBT-202 (new — needs an audit of every `TestClient(main.app)` fixture, real design work).

## 13.1.2: Technical-Debt Sweep — Second Pass — 2026-08-25
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/478 · pyright 0 · pytest 3145 passed/2 skipped · npm compile 0 · eslint 0 · npm test 264 passed · `docker compose up` → healthy
- Shipped: closed DEBT-202 (autouse `tests/conftest.py` fixture patches `autoconnect_enabled_mcp_servers` at both its call sites — `main.py`'s own module-level binding and `tools.mcp_adapter`'s shared attribute `core/task_service.py` re-imports fresh per call — so no test reaches the real, non-isolated MCP catalog; the two files that test the real function directly override the fixture locally), DEBT-179 (`ailienant-core/requirements-dev.txt` split from `requirements.txt`, verified end-to-end: `docker run ... pip list` confirms `pytest`/`mypy`/`ruff`/`hypothesis`/`pre-commit` absent from the built image, `docker compose up` reaches healthy; CI workflow/`CONTRIBUTING.md`/`scripts/pre_commit_backend_gate.py` updated to install both files), and DEBT-120, which turned out to be a bigger defect than its own text described — `core/telemetry.py`'s SQLite connection was never initialized in production (`init_telemetry_db()` had no caller in `main.py`'s lifespan), so three REST endpoints (`/telemetry/routing`, `/telemetry/oom`, `/telemetry/latency`) were permanently empty and DEBT-045's per-action calibration substrate could never accumulate a sample; added the missing lifespan call plus a `purge_old_telemetry()` GC pass wired into `core/janitor.py::run_janitor` alongside vector/graph GC. Ran a project-wide `mypy --warn-unused-ignores` audit: removed 94 stale `type: ignore` comments (6 production, 88 test) mypy proved unnecessary; kept 9 that are platform/optional-dependency-gated (unused on this Windows checkout, load-bearing on the Linux CI runner or without an optional package installed) — one of those nine (`api/byom.py:789`) was initially removed on mypy's word alone and had to be restored once `npx pyright` (a different type checker) flagged it as still needed, a real cross-tool discrepancy now documented in-line. Re-verified DEBT-014's own 6 residual ignores are still all genuinely required. Net: 15 → 12 open.
- Key decision: mypy's `--warn-unused-ignores` is not authoritative by itself — `npx pyright` disagreed on one site the scrub removed, caught only because both gates were re-run before considering the batch done, not just the one that produced the removal list. `--warn-unused-ignores` itself stays off in `mypy.ini` (not promoted to an enforced flag): the 9 kept platform-gated ignores would report as errors on every Windows dev machine while CI (Linux) stayed green, converting a real cross-platform accommodation into a false positive.
- Deferred: none new — both fixes beyond DEBT-202/179 (the telemetry activation and the stale-ignore split) were completed in-batch rather than logged for later, since leaving either half-done would have been worse than not starting (an active write path with no GC, or a scrub with an unverified risky/safe boundary).

## 13.1.3: Output Capacity, Completion Integrity & Live Routing — 2026-08-26
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/475 · pyright 0 · pytest 3206 passed/2 skipped · npm compile 0 · eslint 0 · npm test 264 passed
- Shipped: `docs/OUTPUT_BUDGET_BRIEF.md`'s measured investigation found two reported failures share a family — a marginal context window (real `num_ctx` was pinned at 4096 against a 131072 architectural max, never set explicitly anywhere) and a completion-integrity gap (a corrupted `>>>>>>> REPLACE` marker shipped to disk with nothing validating it). Fixed: the coder's missing truncated-block guard, AST syntax validation wired into `apply_gate` before any write, a runtime capability/window probe (`probe_runtime_capabilities`) replacing guessed constants, real per-call budget arithmetic (deleted a `budget // 2` shape that always collapsed to the same 4096 regardless of input), and native-thinking capability read from the runtime instead of a hardcoded model-name substring list (DEBT-207, third occurrence of DEBT-013's class). Replaced the dead SEQUENTIAL/MICRO_SWARM/FULL_SWARM topology selector (confirmed zero production callers; `brain/intent_router.py`/`swarms.py`/`fast_path.py`/`validators/gates.py` deleted) with the Effort Budget (`light`/`balanced`/`deep`) — a verification-depth axis (lint gate, self-heal ceiling, running the plan's own `checks` via new `run_checks` node) on the one graph every turn already runs, never a which-agents-run axis. Connected the CSS×TCI routing decision — computed, persisted, rendered, never consumed — to the planner/coder's actual model-tier request; deleted the second, drifting routing-matrix implementation. Frontend: `HardwarePanel.tsx`'s hardware-VRAM locks replaced with stated per-level time costs (nothing hardware-locked, since Effort Budget costs local generation time, never VRAM); a new Effort Budget selector in `ModelsMenu.tsx`'s orchestration view.
- Key decision: two live corrections reshaped the plan mid-session — native thinking must stay ON for Companion transparency (the truncation-cause hypothesis was proposed, then refuted by direct runtime probing before shipping); Effort Budget varies verification LAYERS only, all three cognitive agents (researcher/planner/coder) always run regardless of level. §8.1 (incremental plan materialization) was ruled out by measurement rather than judgment: a complete plan costs 764-1550 tokens, marginal at a 4096-token window but not structurally non-viable once the real window is restored, so its contract-wide blast radius wasn't justified by the evidence.
- Deferred: DEBT-203 (no adaptive execution-depth decision exists — every turn pays the full researcher+planner+WBS pipeline regardless of task triviality), DEBT-204 (§8.1/8.3/8.5/8.9 candidate directions, each measured and deferred with its own reasoning), DEBT-205 (`run_checks` executes only the mechanically-executable subset of a plan's `checks`, declared MVP), DEBT-206 (OQ-6 — zero activity-event narration observed during a 12-minute planner window, explicitly out of this task's scope), DEBT-208 (deleting the dead topology modules is a permanent, accepted test-coverage reduction, not a gap to backfill).

## 13.1.4: Developer/Agent Observability Sweep — 2026-08-26
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/475 · pyright 0 · pytest 3212 passed/2 skipped (+6) · npm compile 0 · eslint 0 · npm test 264 passed · `docker compose up` → healthy
- Shipped: closed DEBT-176 (`tool_invocations` emit-only table in `core/telemetry.py`, written from every outcome branch of `core/tool_dispatch.py::ToolDispatcher.dispatch`, added to the 30-day retention sweep in `core/janitor.py` — deliberately NOT wired as a `select_tools` ranking prior, per the backlog's own "rejected, not deferred" clause). New `ERROR` category on `core/telemetry_log.py` (`log_exception`), wired at exactly one canonical boundary (`main.py`'s top-level cognitive-engine failure handler) rather than a mass sweep of the ~98 existing `exc_info=True` sites. `pytest.ini` now emits `test-results.xml` on every run (pytest's built-in `--junitxml`, zero new dependency); CI uploads it as a `backend-test-results` artifact alongside the existing coverage one. `py-spy` added as a dev-only profiling dependency. `DEVELOPERS.md`'s Repository Layout gained the previously-undocumented `telemetry.py`/`telemetry_log.py` lines; `CLAUDE.md` §8/§12 gained one line each naming the new triage/audit surfaces. A companion local-dev MCP server (`ailienant-devtools`, outside this repo) gained a `telemetry` store entry and a `session_digest` tool.
- Key decision: Arize Phoenix (self-hosted OTel/OpenInference tracing) was researched and found genuinely superior to a hand-rolled session-digest tool for real distributed tracing, but deliberately deferred to 13.1.5 (designed, not implemented) rather than folded in here — it adds a new local service and touches the engine's startup/instrumentation boundary, a materially different risk class from this batch's config/logging-only changes.
- Deferred: none new (DEBT-176's ranking-prior half stays rejected, not a fresh deferral).

## 13.1.5: Distributed Tracing (Arize Phoenix) — 2026-08-28
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/476 · pyright 0 · pytest 3219 passed/2 skipped (+7) · npm compile 0 (unchanged) · eslint 0 (unchanged) · npm test 264 passed (unchanged) · `docker compose up` (no flag) → backend-only, healthy · `docker compose --profile tracing up` → backend healthy, Phoenix reachable and receiving real spans (no healthcheck defined for the Phoenix service itself)
- Shipped: `core/observability.py::configure_phoenix_tracing()`/`shutdown_phoenix_tracing()`, opt-in span tracing across LangGraph nodes and every raw `litellm.acompletion()` call (`openinference-instrumentation-langchain` + `-litellm`, client-only in `requirements-dev.txt`). Server is a `profiles: ["tracing"]` `docker-compose.yml` service (`arizephoenix/phoenix:latest`) — a bare `docker compose up` starts only `backend`, verified directly, not assumed. Two corrections to the manifest's original sketch (platform-vs-client package split; litellm needed alongside LangChain for any LLM-level span) are recorded in `docs/SCHEMA_EVOLUTION.MD`'s new "Phoenix Distributed Tracing Dependency Footprint" design decision. `tests/test_observability.py` (7 tests) covers off/unreachable/absent-deps/idempotent/shutdown plus a live litellm-wrapping proof against an in-memory exporter.
- Key decision: a real bug — `register(endpoint=...)` silently skips Phoenix's `/v1/traces` path-suffix logic, 405-ing every export — was caught only because verification ran against a live Phoenix container instead of stopping at green unit tests (which mock `register()` and could not have caught it). Fixed and reconfirmed by a real span landing in Phoenix's own GraphQL API. Recorded as the concrete case for why this project's tests mock the LLM/vector-store boundary but never the thing actually under test.
- Deferred: a measured traced-vs-untraced turn-duration comparison (stated as a real gap in the SCHEMA_EVOLUTION.MD design decision, not fabricated); `@arizeai/phoenix-mcp` registration in the user's own `.mcp.json` (outside this repo, flagged separately, not committed here).

## 13.1.6: Routing-Spine Repair — 2026-08-28
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/475 · pyright 0 · pytest 3229 passed/2 skipped (+25) · npm compile 0 · tsc 0 · eslint 0 · npm test 264 passed
- Shipped: root-caused a live "Drafted a plan but produced no concrete edits" turn to a SUCCESSFUL planner returning a schema-valid, stepless MissionSpecification from a 3B model, and closed the ten defects behind it — a never-computed `task_complexity_index` that collapsed the whole CSS×TCI matrix onto `LOCAL_SMALL`, an unreachable CLOUD tier, a BYOM-blind cloud probe, a missing planner tier floor, no empty-WBS gate, a tier-blind planner cache key, a summary that discarded `errors` on the one path with nothing else to show, three `operator.add` ledgers never reset per turn, a plan-acceptance turn that re-drafted instead of executing, and a grill/synthesis context block assembled with retrieval switched off.
- Key decision: deleted two subsystems rather than logging them as debt. The DriftMonitor was structurally unreachable — `immutable_wbs` was re-seeded `None` every turn, so every plan was compared against itself (similarity 1.0, gate never opened), and its unit tests hand-seeded a differing baseline so the suite never touched the real seed; its premise (cross-turn re-planning) never shipped and its intent is covered twice over by the acceptance panel and the per-step apply gate. The Fast Track probe's only caller was the researcher node, which runs exclusively on coding turns, so its "trivial self-contained question" test could never be true where it was asked — it could only misfire, and did, on the plan-acceptance phrase. Both deletions preceded by a blocking Rewind pre-flight proving LangGraph tolerates a checkpointed channel absent from the compiled schema.
- Deferred: DEBT-209 (CAPABILITY — no way to change the model while a task is running; `InferenceTier` is a frontend-only type with no backend consumer).

## 13.1.7: Graph Config Injection Repair & Transparency Persistence — 2026-08-28
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/476 · pyright 0 · pytest 3237 passed/2 skipped (+8) · npm compile 0 · tsc 0 · eslint 0 · npm test 269 passed (+5)
- Shipped: restored `RunnableConfig` delivery to every graph node. Both node wrappers were variadic, which advertises no injectable parameter, so LangGraph passed nothing and every `config.configurable` seam — narration, the reasoning sink, the activity channel, the cell dispatcher, the compaction callback — was None inside every node; the one timeline row users saw was the marker `task_service` emits outside the graph. Fixed by declaring `config` explicitly on `_instrument_node`/`_guarded` and forwarding it via a shared `accepts_config`, corrected `agentic_cell`'s rejected annotation, and closed the grill's reasoning pass, which never read `stream_thinking` at all. Because the restored trace multiplies per-turn timeline volume, reasoning now persists (bounded, clock settled) instead of being stripped, the in-flight snapshot sheds recoverable bodies and is budget-trimmed, `companions` reaches the durable transcript, and one oversized slot can no longer lose the whole `setState` envelope.
- Key decision: `_instrument_node` deliberately does NOT use `functools.wraps`. It would set `__wrapped__`, `inspect.signature` follows it, and LangGraph would read the inner node's signature again — putting delivery back at the mercy of each node annotating `config` exactly right, which is the second half of the same bug. Keeping the wrapper opaque makes it the single place that guarantees the seam.
- Deferred: none. A follow-up on the grill's pinned tier was drafted and then withdrawn: the pre-routing pins are a consistent policy (capability allocated by blast radius × recoverability — a human reviews every grill round, nothing reviews the one-shot distillation, the planner's shape gates the turn, a coder step has validators behind it), not drift.

## 13.1.8: Reviewable Distillation Brief — 2026-08-28
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/476 · pyright 0 · pytest 3247 passed/2 skipped (+10) · npm compile 0 · tsc 0 · eslint 0 · npm test 276 passed (+7)
- Shipped: the brief that closes a Socratic interview is now shown in chat before it becomes the planner's input — accept (edits authoritative), rewrite with a note, or cancel. It was the only stage in the pipeline nothing checked: the coder has validate_output plus acceptance checks, the planner its Actor-Critic loop, the grill a human every round, while the distillation replaced `user_input` unreviewed and the critic validates the resulting plan against a schema, never against the dialogue. `run_synthesis_node` became two super-steps on a new `synthesis_node` self-loop, and the whole review rides the existing approval substrate — `request_graph_approval` already returned approved/comment/modified_content and `useHitlResponder` already posted all three, so no new wire message and no new resume path.
- Key decision: the two-phase split is load-bearing, not tidiness. LangGraph replays a node from the top on every resume, so drafting and interrupting in one invocation would re-run the MODEL_BIG distillation on every review round — charging again and swapping out the very text under review for one the operator never saw. The gate for it drives both super-steps and asserts the distillation ran exactly once; a single-phase regression passes a one-shot test and fails this one.
- Deferred: none. A simplification landed instead of a wire addition: `_compose_planner_brief` already emits the settled constraints and scope as labelled blocks, so rendering the composed brief verbatim shows them as lists — no extra payload fields, and what the operator reads stays exactly what the planner receives.

## 8.19: Web Research Capability Parity & Outbound-Fetch Hardening — 2026-08-28
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/478 · pyright 0 · pytest 3278 passed/2 skipped (+31)
- Shipped: `web_fetch` now reaches all 8 dev roles plus the Researcher and `web_search` reaches the Researcher, secops, and devops_infra — closing a PLAN-mode gap where the node that owns retrieval could not read a URL at all. New `core/url_guard.py` validates every outbound destination (scheme allowlist; loopback, RFC1918, link-local/cloud-metadata, reserved and internal-suffix hosts refused; loopback alone operator-opt-in) and `WebFetchTool` now walks redirects manually so each hop is re-validated, streams the body under its cap, re-caps after Markdown conversion, budgets calls per turn, redacts URLs before logging, and emits a `NETWORK` audit line on every denial.
- Key decision: the Planner was left tool-less on purpose. `agents/researcher.py` already documents the retrieval/decision split — the Researcher grounds, the Planner decides — so the capability belongs in the Researcher, and routing it through 8.15's subagent dispatch would have rebuilt with heavier machinery what that split already provides. The audit that preceded the role unification also found `analyst_readonly` holding no role in any tool: `subagent_worker_node` hands the critic the analyst arsenal, but `classify` denied every call it made. Latent only — `ENABLE_DYNAMIC_DISPATCH` defaults off — and repaired here before that flag is ever turned on.
- Deferred: DEBT-213 — validate-then-connect resolves DNS twice, so a rebinding attacker can still differ the two answers; closing it needs address pinning that preserves TLS SNI, not a stricter predicate. DEBT-214 (DuckDuckGo fallback parses unversioned markup) and DEBT-215 (`allowed_tools` vestigial but frozen by a test snapshot) also logged.

## Tooling: Engineering Metrics Gate (Complexity, Duplication, Debt Ratio) — 2026-08-28
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/478 · pyright 0 (unchanged) · pytest 3278 passed/2 skipped (unchanged, no application code touched) · npm compile 0 · eslint 0 errors/22 pre-existing warnings surfaced by two new advisory rules
- Shipped: an advisory-only (`continue-on-error: true`, same pattern as `openspec-gate.yml`) `code-metrics-gate.yml` running `radon cc`/`radon mi` (backend) and `jscpd` (`npm run metrics:duplication`, both zones, `.jscpd.json`) on every push/PR; two `"warn"`-severity ESLint rules (`complexity`, `max-lines-per-function`), explicitly carved out of the zero-degradation policy. `docs/TECH_DEBT_BACKLOG.md` gained a Debt Ratio section with a real computed baseline (0 HIGH/3 MEDIUM/13 LOW weighted 19, SLOC 40,446 via `radon raw` → ≈0.47/KLOC) and DEBT-216 (Capability Backlog — agentic product-metric rollup, deliberately not built, see Key decision). New `CLAUDE.md` §16 scopes what applies to a solo-dev, local-first project (complexity/duplication/MI, debt ratio, on-demand DORA-lite, CI-reliability SLO) versus what doesn't (full-team DORA, customer SLAs, product analytics) and states the zero-degradation carve-out explicitly so it doesn't silently contradict §8.
- Key decision: agentic product metrics (Task Completion Rate, Tool Call Accuracy, Cost/Task, Self-Correction Rate) were investigated and confirmed as real production practice (Phoenix/LangSmith/Galileo all ship this), but logged as DEBT-216 rather than built — it instruments `core/`/`brain/` runtime behavior, not dev-process code quality, is a different axis from this sweep's scope, and needs its own blueprint given the higher blast-radius zone (CLAUDE.md §3).
- Deferred: DEBT-216 (agentic product-metric rollup, unscheduled).

## 13.1.9: Agent Attribution & Living Timeline — 2026-08-28
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/478 · pyright 0 · pytest 3292 passed/2 skipped (+14) · tsc 0 · eslint 0 errors · npm test 297 passed (+12)
- Shipped: the Glass-Box Timeline now attributes every row to the agent that produced it (consecutive lanes, replacing 13.0.7's phase grouping) with a model-tier badge, gives a tool call its own `tool` kind instead of reading as an indistinguishable shell command, and replaces two competing "what's happening" surfaces (a sticky-header status line + the timeline itself) with one live loader row that shows real streaming reasoning or the newest real action — cycling through equivalent phrasings for the three kinds with no concrete detail, never inventing an event. The rail moved from a per-row border (which broke at every group boundary) to one continuous pseudo-element, making that defect structurally impossible.
- Key decision: attribution rides two contextvars with two precedences — an outer default the node wrapper derives from its own name, and a narrower override `ToolDispatcher.dispatch` binds for one call. The coder needed a THIRD wrinkle, a new `activity_role` constructor parameter distinct from `active_role`: its RBAC identity is the per-WBS-step target_role, real for permissions but not what should split one coding turn into a new lane every step.
- Deferred: 13.1.10 (Reviewable Model Route) is the planned next sub-phase, not deferred debt — the router's decision is visible via this phase's lane badge; making it a reviewable HITL gate is separately scoped work.

## 13.1.10: Reviewable Model Route — 2026-08-28
**Status:** COMPLETE | **Gates:** ruff 0 · mypy 0/480 · pyright 0 · pytest 3301 passed/2 skipped (+9) · tsc 0 · eslint 0 errors · npm test 309 passed (+12)
- Shipped: once per turn, before the planner drafts, the router's model pick is now a decision, not just a display — accept, override to a different tier from the active preset, or cancel, with TCI/CSS shown as the justification. `run_model_route_node` sits between researcher_agent and planner_agent in both graph topologies, reusing the exact same approval substrate 13.1.8's brief review already proved out — no new wire message, no new resume path. `confirmed_routing_decision` is what the planner and coder read instead of the raw router verdict, which is what makes one confirmation cover every WBS step of a turn.
- Key decision: the plan called for a two-phase defer-then-interrupt-first node mirroring the brief review's split. Implementation verified that assumption against the actual cost and corrected it — the brief review needs two phases because drafting is an expensive MODEL_BIG call that must not silently re-run on replay; resolving a routing decision is a pure re-read of state already sitting there, so a single-phase node is both simpler and equally safe. Also corrected: "AUTO gates" was written against a fictional 3-value mode axis; the real `SessionPermissionMode` has seven members, and the gate now checks membership against the two the 3-button UI's Ask/Plan actually produce, not a guessed name.
- Deferred: none.
