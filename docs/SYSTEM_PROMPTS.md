...
**`_STACK_GUIDANCE_DIRECTIVE`:** An ordered procedure, not a catalog of named frameworks (a hardcoded stack list would cost tokens on every call, go stale, and override the model's own knowledge). For requests that don't specify a technology: use whatever the user named, or match a stack already visible in the workspace; otherwise classify the artifact class in the model's own words, name the constraints that genuinely narrow it, and choose what a practitioner building that class of artifact would reach for (a game implies a game engine, not a web framework) — recording the outcome as the first entry of `MissionSpecification.decisions` in a fixed `Stack: <names> — <reason>` form, with every WBS `target_file` required to stay consistent with it. Ambiguity still gets a concrete, recorded choice rather than a silent default to a generic web stack. That `decisions` entry is then propagated to both code generators via `MissionSpecification.to_context_block()` (`brain/state.py`) — consumed by `agents/coder.py`'s budget-guarded execution block and re-injected into `brain/agentic_cell.py`'s ReAct loop every iteration — so the choice cannot decay across a multi-step build.
...
`agents/analyst.py::run_analyst_node` (the `analyst_grill` node of `brain/ideation.py`) interviews the user before planning. Each round asks a **batch** of every question it currently needs answered — not one at a time — and is now TWO graph super-steps (`brain/ideation.py`'s self-loop drives both), not one:

1. **Generate phase** (`pending_grill_batch` empty): narrates `grill_grounding`, then `_assemble_socratic_context`/`_gather_tool_grounding` ground the round in the workspace. Narrates `grill_composing_questions`, then `_generate_grill_questions_llm` makes one structured call returning a `GrillQuestionBatch` (`tools/control_tools.py`), reusing `AskUserQuestionItem`/`AskUserQuestionOptionInput` so the grill and `ask_user_question` share one question/option model. `_GRILL_DIRECTIVE` instructs 2-6 questions per batch, each with 2-4 concrete mutually-exclusive options and exactly one `recommended`.
   - An **empty `questions` list is the completion signal** — the model decides it has enough shared understanding, which sets `shared_understanding_reached=True` and hands off to `synthesis_node`. The legacy free-text agreement-phrase check (`_AGREEMENT_SIGNALS`) remains only as a fast path for a genuinely new top-level turn (`grill_round_count == 0`).
   - A non-empty batch is **committed to `pending_grill_batch`** (a declared state channel) and the node returns — no `interrupt()` call in this invocation.
2. **Ask phase** (`pending_grill_batch` set, the self-loop's next visit): `_resolve_grill_answers` on the state-sourced batch is the node's FIRST action, suspending via `request_graph_clarification(questions=...)` (native `interrupt()`) on the same channel `ask_user_question` uses — rendering as `ClarificationGrillCard`. Folds the resumed answers into `messages` and clears `pending_grill_batch`.

This split exists because LangGraph replays a node from the top on resume: generating the batch and calling `interrupt()` in one invocation (the original 13.0.5 shape) meant a resume could silently regenerate a *different* batch than the one the operator answered (`SCHEMA_EVOLUTION.MD §53`, DEBT-185). `route_after_analyst` loops back to `analyst_grill` in both phases — the edge only decides whether another round is needed, never whether to suspend. `grill_round_count` bounds the loop at `_GRILL_MAX_ROUNDS`.

The batch is not streamed token-by-token — a structured JSON call is inherently non-streaming, matching the planner's own structured generation. `_generate_grill_questions_llm` fails fast (returns `None`, distinct from an empty batch) when no BYOM chat target resolves, rather than falling through to the litellm proxy and burning the transport-retry budget; the node then surfaces an actionable notice and ends the turn (`hitl_pending=True`, which `route_after_analyst` checks first and routes to `END`, never the self-loop).

`None` is not one condition, and the notice names which one it was. An unreachable engine and a model that answered with unusable JSON need opposite responses from the operator (start a model, versus pick a larger tier), so the generator records the cause in its `failure` out-param and `_grill_failure_message` maps it. Reporting both as "I can't reach the configured model" — the behaviour before this was split — sent operators to restart an engine that was already running and answering.
