...
> Bloque previo al ciclo de pruebas/refinamiento. Objetivo: `mypy --strict main.py` → **exit 0**, cero entradas `follow_imports = silent` en `mypy.ini`. WBS completo en [`docs/PHASE_8_BLUEPRINT.md`](PHASE_8_BLUEPRINT.md); deuda técnica continua en [`docs/TECH_DEBT_BACKLOG.md`](TECH_DEBT_BACKLOG.md). Baseline re-medido (2026-06-05, tras 8.0.1): **7 errores residuales** en 3 archivos (todos tras `tools.llm_gateway` → 8.0.2), 6 módulos silenciados. El gate `mypy .` (247 archivos) permanece verde durante toda la campaña.
...
- [ ] **8.0.2 — Liberar `tools.llm_gateway`** — desbloquea summarizer, contract_guard, coder, rutas BYOM. Resolver DEBT-002/015 (`MODEL_MEDIUM` no exportado). **DoD:** `mypy --strict tools/llm_gateway.py` → 0; entrada eliminada.
...
- [ ] **8.0.4 — Nodos Tier 2/3 desbloqueados** — summarizer (DEBT-016 residual), coder (DEBT-014), trajectory_memory, swarms, intent_router. Ejecutar en orden topológico. (`ideation.py` ya resuelto en 8.0.1.) **DoD:** `mypy --strict <file>` → 0 para cada nodo.
