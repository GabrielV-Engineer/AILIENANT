...
| Phase 11 Dashboard Enterprise Redesign | 🟡 Active | — | 11.4 BYOM & Extensions polish (11.0–11.3 shipped; 11.3.B backend-telemetry deferred) |
...
- [ ] **11.3.B — Monitoring Backend Telemetry (unblocks the deferred 11.3 charts).**
  [Spec: add the persisted telemetry the frontend-only 11.3 could not fabricate. (1) Per-request latency capture + P50/P95 rollup — instrument the graph/gateway request path; surface in `TelemetryPanel`. (2) Docker lifecycle event log (timestamped container start/stop transitions) → `RuntimePanel` Gantt. (3) Adapter set-tier endpoint + validation → `RuntimePanel` manual tier switcher. (4) Bounded live container log-stream endpoint → `RuntimePanel` log view. Closes DEBT-116, DEBT-117, DEBT-118, DEBT-119.]
