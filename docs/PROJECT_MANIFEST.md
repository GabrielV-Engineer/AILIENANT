...
- **Hito Reciente:** 7.13.9 COMPLETA — Orphanage Recovery I: Máquina de Estados Multi-Turno & Planner UI (ADR-713): nuevo eje de superficie `surface: 'chat' | 'planner'` (persistido, ortogonal al `mode` de ejecución); `ModeSwitcher.tsx` + `PlannerSession.tsx` (formulario Socrático multi-turno bloqueado que ahora alcanza el `ideation_loop` del backend). Cableado **frontend-only** vía flag aditivo `planner_mode_active` en el payload de submit (cero cambios de Python). Corregido el cross-wiring de `dreaming_toggle` que activaba el modo Planner Socrático por error. Tarjeta estructurada de `MissionSpecification` diferida a Fase 4. **748 tests verdes (sin Python tocado).**
...
- **Próximo Objetivo:** 7.13.10 — Orphanage Recovery II: Surface Sync & Push-Fed Panels (ADR-716)
