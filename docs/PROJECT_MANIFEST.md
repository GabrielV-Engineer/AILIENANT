...
> Bloque previo al ciclo de pruebas/refinamiento. Objetivo: `mypy --strict main.py` → **exit 0**, cero entradas `follow_imports = silent` en `mypy.ini`. WBS completo en [`docs/PHASE_8_BLUEPRINT.md`](PHASE_8_BLUEPRINT.md); deuda técnica continua en [`docs/TECH_DEBT_BACKLOG.md`](TECH_DEBT_BACKLOG.md). Baseline re-medido (2026-06-05): **15 errores residuales** en 4 archivos (todos tras deps silenciadas), 9 módulos silenciados. El gate `mypy .` (247 archivos) permanece verde durante toda la campaña.
...
- [ ] **8.0.4 — Nodos Tier 2/3 desbloqueados** — summarizer, coder (DEBT-014), trajectory_memory, ideation (DEBT-016), swarms, intent_router. Ejecutar en orden topológico. **DoD:** `mypy --strict <file>` → 0 para cada nodo.
