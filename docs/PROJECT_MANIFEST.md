...
- [ ] **11.2.S — Symbol-Level Node Types (deferred from 11.2).**
  Extend the nebula taxonomy to `function`/`class`/`module` node shapes. Blocked on the `SCHEMA_EVOLUTION.MD` "Symbol-Level Call-Graph Substrate" decision, which deliberately keeps `symbol_definitions`/`observed_call_edges` out of the analytics graph (would exceed `MAX_GRAPH_EDGES=5000`); requires amending that decision + a symbol-edge analytics pipeline first. See DEBT-111.
