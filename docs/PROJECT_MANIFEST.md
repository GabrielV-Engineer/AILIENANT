...
| Phase 12 Pre-Launch Innovation Sprint | 🟡 Active | — | 12.5 Quality & Polish Debt Sweep (12.1 shipped narrowed to prefix stability; 12.2 accelerated/closed early; 12.3 closed both DEBT entries corrected, not literally implemented; 12.4 shipped 083/085/086 fully + 084 as plumbing-only, DEBT-138 logged for the deferred cell reroute) |
...
- [ ] **12.13 — Symbol-Level Chunk Embeddings (closes DEBT-140).**
  GraphRAG embeds one vector per whole file, no chunking — a large multi-function file collapses to a single centroid resembling none of its individual functions, capping retrieval precision independently of embedding model quality. Hybrid by size: files under a size threshold keep today's single file-level vector unchanged; files over it additionally emit per-symbol chunk rows in a new, additive LanceDB table (never mutate `workspace_embeddings`' Arrow schema — CLAUDE.md §10), reusing the already-populated `symbol_definitions` catalog (`core/ast_engine.py::collect_symbol_defs`, `SCHEMA_EVOLUTION.MD §27`) rather than re-parsing. **Must carry, as an explicit WBS item, the migration of DEBT-142's query-time skeleton distillation to index time** — once chunk rows exist, that evidence is stored, not recomputed per retrieval, which removes the O(K·N) hot-path cost, the shared-thread-pool exposure, and both containment layers 12.11 introduced, all at once; tree-sitter already runs in `core/compute_pool.py` on the indexing path, so this returns the work to the process pool where it belongs. **Must name the `_MIN_TOKENS = 100` interaction:** that anti-fragmentation gate is calibrated for whole files, and most individual functions fall under 100 tokens — chunks need their own, lower threshold or the feature silently writes nothing. **Must update the same change:** `core/janitor.py::_vector_gc_sync` (orphan GC currently covers only `workspace_embeddings`), `semantic_delete` (must purge both tables or deletes leave chunk ghosts), the dimension-mismatch drop/recreate path (`semantic_memory.py:298-307`), and the `SCHEMA_EVOLUTION.MD §34` dashboard surface (`list_embeddings`, `pca_project_2d`, purge endpoint) — each silently rots otherwise. Adoption requires a full reindex — state as a migration note, not a silent expectation. Record the counter-intuitive cost result: incremental cost *improves* for large files (editing one function re-embeds one function, not the whole file); only the initial index of over-threshold files costs more. DoD: `mypy .` 0 · `pyright` 0 · `pytest` green.
- [ ] **12.10 — Pre-Launch Innovation Gate.** Prompt caching tokens-saved metric > 0; DEBT-049/054/051(already closed)/083(already closed)/084(already closed)/085(already closed)/086(already closed)/045(already closed, 12.5)/047(already closed, 12.5)/052(already closed as INVALID, 12.5)/067(already closed, 12.5)/012(already closed, 12.5)/079(already closed, 12.5)/097/100/106/129/130/127(conditional)/121/123/124/125/126/128/132/133/134/141(already closed, 12.11)/142(already closed, 12.11)/143(already closed, 12.11)/144(already closed, 12.12)/140(12.13) closed (DEBT-014 closed only if unblocked — re-measured 2026-08-03 at 12.5 and still blocked, 78 errors/24 files; DEBT-122 has an explicit logged decision, not necessarily closed; DEBT-138 stays open — blocked on an OCC-safe sync surface, not a 12.x gate item; DEBT-139 is floating, not required for this gate; DEBT-145 is a new floating item logged at 12.5, not required for this gate; DEBT-146 is a new floating item logged at 12.12, not required for this gate); manifest and backlog ledger rows internally consistent (12.9); `pytest` green · `mypy .` 0 · `npm run compile` 0.
