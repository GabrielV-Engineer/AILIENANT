...
- **Hito Reciente:** 7.13.5 COMPLETA — reactive track: entrada unificada idempotente por content-hash (`indexed_files.content_hash`), project_id real cableado al path reactivo, circuit breaker per-(project,file) (GAP6), purga de grafo+vector en delete/rename, fuga `O(C)` de sesión corregida. Cierra el enrichment track previo (confidence edges, Louvain, God Nodes). 722 tests verdes.
- **Próximo Objetivo:** 7.13.6 — Manual Dreaming: acción "Consolidate Memory" (ADR-710, sólo manual)
