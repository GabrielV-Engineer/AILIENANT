...
- **Hito Reciente:** 7.13.11 COMPLETA — Zero-Deduplication Sweep: consolidados los **3** lectores VFS de agentes (coder + analyst_context + error_correction — el lector vivo del analista NO está en `analyst.py`) en una única factory `core/vfs_middleware.py::make_safe_reader`. Corregido un bug colateral: `prompt_builder._read` devolvía siempre None (`isinstance(VFSReadResult, str)`) — código muerto, ahora correcto. Retry/backoff finalizado: constantes `LLM_MAX_TRANSPORT_RETRIES`/`WAL_CHECKPOINT_MAX_RETRIES` en `retry_policy.py` referenciadas por el gateway (7×) y `db_maintenance`. Fence ISO1 intacto. **748 tests verdes; mypy 224 OK.**
...
- **Próximo Objetivo:** 7.13.12 — Checkpoint Gate Fase 7.13 (verificación integral SC/PR/CC/RL/SF/CN/DR/AL/FR/OR/DB/TL/DD/REG)
