...
- [ ] **12.12 — Dead Context-Assembler Reclamation (closes DEBT-144).**
  `brain/prompt_builder.py` (~330 lines: token-budget-aware context assembler with flesh/skeleton tiering) has zero callers anywhere in production — verified by a full name sweep of `PromptBuilder`, `ContextBundle`, `build_context`, `build_system_prompt`. Its file selection is global-PPR (`get_top_ppr_files` — "the project's most central files"), which is query-blind and would inject the same files into every turn regardless of the question; wiring it in as-is is not the fix. 12.11 already harvested the module's two valuable mechanics (real token-budget accounting, flesh/skeleton tiering) into the query-relevant retrieval path via DEBT-142, so this sub-phase is pure deletion, not a rewrite. Delete `brain/prompt_builder.py`; update `DEVELOPERS.md`'s Repository Layout. **Hard constraint:** `tests/test_phase7_13_checkpoint_gate.py::test_dd1_single_vfs_reader_and_named_retries` reads `brain/prompt_builder.py` as raw text (`assert "read_safe(" not in pb`) — retarget or drop that assertion in the same change, or the gate fails with `FileNotFoundError` on deletion. `build_system_prompt` is separately superseded by 12.1's `build_static_identity_prompt`/`build_boundary_declaration` split in `agents/prompts.py` — confirm no remaining caller expects its old behavior before removing it. DoD: `mypy .` 0 · `pyright` 0 · `pytest` green.
...
- [ ] **12.10 — Pre-Launch Innovation Gate.** Prompt caching tokens-saved metric > 0; DEBT-049/054/051(already closed)/083(already closed)/084(already closed)/085(already closed)/086(already closed)/045/047/052/067/012/079/097/100/106/129/130/127(conditional)/121/123/124/125/126/128/132/133/134/141(already closed, 12.11)/142(already closed, 12.11)/143(already closed, 12.11)/144(12.12)/140(12.13) closed (DEBT-014 closed only if unblocked; DEBT-122 has an explicit logged decision, not necessarily closed; DEBT-138 stays open — blocked on an OCC-safe sync surface, not a 12.x gate item; DEBT-139 is floating, not required for this gate); manifest and backlog ledger rows internally consistent (12.9); `pytest` green · `mypy .` 0 · `npm run compile` 0.
