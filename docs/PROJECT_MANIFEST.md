...
> Bloque previo al ciclo de pruebas/refinamiento. Objetivo: `mypy --strict main.py` → **exit 0**, cero entradas `follow_imports = silent` en `mypy.ini`. WBS completo en [`docs/PHASE_8_BLUEPRINT.md`](PHASE_8_BLUEPRINT.md); deuda técnica continua en [`docs/TECH_DEBT_BACKLOG.md`](TECH_DEBT_BACKLOG.md). Baseline re-medido (2026-06-05, tras 8.0.3): **1 error residual** en `mypy --strict main.py` (swarms:155, DEBT-014 — discrepancia `NodeInputT` strict/non-strict), 3 módulos silenciados (`core.db`, `api.websocket_manager`, `brain.memory`). El gate `mypy .` (247 archivos) permanece verde durante toda la campaña.
