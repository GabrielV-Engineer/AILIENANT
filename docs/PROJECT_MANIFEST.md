...
> Bloque previo al ciclo de pruebas/refinamiento. Objetivo: `mypy --strict main.py` → **exit 0**, cero entradas `follow_imports = silent` en `mypy.ini`. WBS completo en [`docs/PHASE_8_BLUEPRINT.md`](PHASE_8_BLUEPRINT.md); deuda técnica continua en [`docs/TECH_DEBT_BACKLOG.md`](TECH_DEBT_BACKLOG.md). Baseline re-medido (2026-06-08, tras 8.0.4): **`mypy --strict main.py` → 0** (objetivo primario alcanzado). Quedan 3 módulos silenciados (`core.db`, `api.websocket_manager`, `brain.memory` → 8.0.5/8.0.6) y 3 `# type: ignore[type-var]` residuales en swarms/ideation (DEBT-014, no bloqueantes). El gate `mypy .` (247 archivos) permanece verde durante toda la campaña.
