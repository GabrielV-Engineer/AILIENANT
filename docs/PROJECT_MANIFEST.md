...
- **Hito Reciente:** 7.13.7 COMPLETA — Self-Healing: `ErrorCorrectionAgent` (traceback → lee archivo → propone fix HITL → reintenta ≤3) cableado en **ambos** el grafo (`brain/engine.py`, `reflexion_guard` dentro del DLQ) y el bucle vivo de coders (`core/task_service.py`); `brain/retry_policy.py` (presupuestos centralizados) + `brain/failure_breaker.py` (breaker de firma cross-turn, GAP8); panel **Recovery** cablea los huérfanos `/dlq/pending` + `/task/resume`. 743 tests verdes.
...
- **Próximo Objetivo:** 7.13.8 — Frontend Stream Resilience & Lifecycle Re-attach (ADR-715)
...
- [ ] **7.13.8 — Frontend Stream Resilience & Lifecycle Re-attach** *(fundacional para superficies, NUEVO · ADR-715)*
