...
- [ ] **1.x. Integración de Motor AST en el VFS (`Tree-sitter`):**
...
- [ ] **1.x.1 Tablas de Estado y Catálogo en SQLite (`core/db.py`):**
...
- [ ] **2.0.1 PlannerAgent y Lógica de Ruteo Condicional (MoE Híbrido y Model Cascading):**
...
- [ ] **2.0.2 Topología Avanzada LangGraph (Modo "MapReduce" para High-TCI):**
...
- [ ] **2.1. Matriz de Enrutamiento 3D y Tokenización:**
...
- [ ] **2.1.X.1. Implementación de Backpressure en WS:**
  - [ ] Crear `transport/throttler.py` para monitorear el `write_buffer_size` del servidor FastAPI.
...
- [ ] **2.1.X.3. Implementación del Shadow Planner:**
  ...
  - [ ] crear Nodo DriftMonitor en LangGraph para comparar estado actual vs immutable_wbs inicial.
  - [ ] **HITL (Human-in-the-loop) Gate:** Interfaz de WebSocket para que el usuario valide desviaciones del plan.
- [ ] **2.1.X.4. Implementación de Shallow State y Storage Provider (Anti-Bloat VFS):**
  - [ ] **Refactorización de VFSFile:** Modificar `SCHEMA_EVOLUTION.MD` para eliminar el campo `content: str`. Reemplazarlo por `blob_hash: str` (Puntero).
  - [ ] **Middleware de Persistencia Volátil:** Crear `core/blob_storage.py` (Un almacén RAM/Redis). Cuando el IDE envíe un `dirty_buffer` completo, el backend guarda el texto en el `blob_storage` y solo inyecta el `blob_hash` en el estado de LangGraph.
  - [ ] **Soporte para Delta Updates (Diffs):** Modificar el contrato del `CoderAgent` para que emita parches (Unified Diff) en lugar de retornar archivos completos al Grafo.
