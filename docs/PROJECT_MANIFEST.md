...
- [ ] **8.14.6.1 — Two-Tiered symbol substrate implementation (conditional on 8.14.6 GO).**
...
- [ ] **8.15.0 — Structured dispatch schema.**
  New `brain/subagent_contracts.py`: Pydantic `SubagentTask`, `SubagentResponseSchema` (closed field-type vocabulary, not a free-form dict), `DispatchPlan` (`dispatch_depth` bound `[0,2]`, `tasks` bound `[1,32]`), `SubagentResultEnvelope` (`raw_digest` capped at 4000 chars, reusing `core/tool_dispatch.py`'s `_MAX_OBSERVATION_CHARS`), `DispatchBatchResult`. Four additive `AIlienantGraphState` channels: `dispatch_plan`, `dispatch_batch_result`, `dispatch_depth`, `subagent_dispatch_trace` (`operator.add`). New `SCHEMA_EVOLUTION.MD §27` entry — all four channels default-safe (`None`/`0`/`[]`) so pre-existing checkpoints deserialize unchanged. **DoD:** `pytest` (schema validation incl. depth/width bound rejection) · `mypy .` 0 · `npx pyright` 0.
- [ ] **8.15.1 — Generalized Send() dispatch primitive.**
...
- [ ] **8.15.2 — Dispatch synthesis + wave batching.**
...
- [ ] **8.15.3 — Tournament module extraction.**
...
- [ ] **8.15.4 — Budget admission + concurrency wiring.**
...
- [ ] **8.15.5 — Six-pattern wiring + recursion/permission guards.**
