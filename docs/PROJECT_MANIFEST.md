...
- [ ] **8.5.5 — Eval surface tools** — ▶️ **DESBLOQUEADO** (8.3.5 cerró 2026-06-13; División 8.3 completa)
  ...
  - ✅ **Dependencia satisfecha:** el productor existe — `tests/benchmark/report.py` (`build_report`/`write_report`) + `BenchmarkRunner.run_report` emiten el `report.json` validado contra `tests/benchmark/report.schema.json`. `get_report` lee ese artefacto; `run_benchmark` envuelve `run_report` (budget-gated, async).
