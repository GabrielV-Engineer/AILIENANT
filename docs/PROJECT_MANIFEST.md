...
- **Hito Reciente:** 7.13.8 COMPLETA — Frontend Stream Resilience & Lifecycle Re-attach (ADR-715): dedup idempotente server-side (`TaskPayload.request_id` + caché TTL en `submit_task`); **stream watchdog dinámico Zero-Config** (timeout gobernado por el backend según modelo local/nube vía `byom_config.stream_watchdog_ms()`, inyectado en la respuesta de submit); ACKs aditivos `server_abort_ack`/`server_hitl_ack` (Stop con WS caído ya no congela la UI; HITL no se orfana); `isAborting` limpiado en rehydrate; chips colgados normalizados; caps duros en tool-output (500) e inline-edit queue (2000); `document_version_id` sembrado al conectar. **748 tests verdes.**
...
- **Próximo Objetivo:** 7.13.9 — Orphanage Recovery I: Máquina de Estados Multi-Turno & Planner UI (ADR-713)
...
- [ ] **7.13.9 — Orphanage Recovery I: Máquina de Estados Multi-Turno & Planner UI** *(ADR-713)*
  ...
  - **Resolution:** `src/workspace/Workspace.tsx` → máquina de modos (Chat / Planner / Dreaming Dashboard); nuevo `PlannerSession.tsx` con formulario multi-turno bloqueado; **cablear el sender huérfano `client_planner_mode_toggle`** (el tipo existe en `config.ts`, sin emisor); consumir los eventos de ideación existentes; `MissionSpecification`/`AIlienantGraphState` sin cambios.
...
- [ ] **7.13.10 — Orphanage Recovery II: Surface Sync & Push-Fed Panels** *(ADR-716)*
...
- [ ] **7.13.12 — Checkpoint Gate Fase 7.13**
