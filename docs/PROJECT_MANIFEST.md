...
- [ ] **11.5.C — Agent Activity Timeline (Glass-Box Transcript).**
...
- [ ] **11.5.C.4 — Tool/Cell Timeline Migration (closes DEBT-122).**
  Complete 11.5.C's "subsumes the six widgets" goal: migrate Rich Tool Chips (`ToolChip`/`ActionLog`) and the agentic-cell audit (`CellAuditWidget`) from unmigrated siblings into `AgentTimeline` rows, matching the reasoning/plan/diff migration 11.5.C.0–.3 already shipped. Requires a new backend activity-marker emission point (`ref = tool_call_id` for `server_tool_start`, an iteration ref for `server_cell_*`) — the coder's `run_command` narration path (already correlated) is a *separate* mechanism from Rich Tool Chips' tracked-tool registry, so this is new wiring, not a relabel. `upsertToolBody` (frontend, `timelineBuilder.ts`) is already implemented and unit-tested — this task supplies its producer and the `AgentTimeline` row rendering (reuse `ToolChip`/`CellAuditWidget` as node bodies, same pattern as the `DiffBlock`/`ExecutionChecklist` rows). Once this lands, flip `11.5.C` to `[x]`. Files: `ailienant-core/api/ws_contracts.py`, `core/task_service.py`, `tools/execution_tools.py` (or wherever tool-start fires); `ailienant-extension/src/workspace/components/AgentTimeline.tsx`, `hooks/useWSMessageHandler.ts`. **DoD:** `ToolChip`/`CellAuditWidget` render as timeline rows correlated by real backend refs (order-agnostic, same guarantee as diff/reasoning); the standalone sibling renders are removed; `pytest`/`mypy`/`pyright`/`npm test` green.
