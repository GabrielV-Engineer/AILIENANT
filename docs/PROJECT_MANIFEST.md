...
- **Hito Reciente:** 7.13.10 COMPLETA — Orphanage Recovery II: Surface Sync & Push-Fed Panels (ADR-716): **inventario gated aprobado** (§5.2 del blueprint). Re-auditoría confirma que los 4 paneles fetchean endpoints reales; corrección arquitectónica — el dashboard es una página HTTP servida por el backend (sin WebSocket/host bridge) y los paneles se desmontan al cambiar de pestaña, así que **no hay leak de polling**. Hardware/Runtime pasan a poll **visibility-gated** (nuevo hook `usePollingWhileVisible`) en vez de un bus WS (over-engineering). Huérfanos: `master_toggle`/`profile_change` FE eliminados (handlers backend retenidos); OOM cableado vía nuevo evento aditivo `server_oom_engaged` (broadcast best-effort desde `_oom_cascade`, ruteado por `task_id`) — el toast `OOM_ENGAGED` muerto ahora dispara de verdad; terminal de `ContextOverlay` verificado (manual by design). **748 tests verdes; mypy 224 OK; check-types/lint/compile OK.**
...
- **Próximo Objetivo:** 7.13.11 — Zero-Deduplication Sweep (consolidar lectores VFS de `coder.py` + `analyst.py`)
