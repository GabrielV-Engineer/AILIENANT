...
7. **No Hardcoding — Derive, Configure, or Name:** A hardcoded value is a rejected mutation when any of the three alternatives below applies. In descending order of preference:
   - **Derive it.** If the value already exists somewhere authoritative — a config file, an enum, a schema, another module's constant, a computed signal — read it from there. Two literals that must agree are two sources of truth, and they *will* drift. A capability list that already exists (`_TIER_ORDER`, a `Literal`, a preset's own tiers) is the source; never restate its members inline.
   - **Configure it.** A value an operator or deployment may legitimately need to change (a model alias, a path, a budget, a URL, a feature gate) belongs in `shared/config.py` behind an `os.getenv` default, or in the relevant settings/preset file — never inline at the call site.
   - **Name it.** When a literal genuinely has no upstream source and no operator would tune it, it MUST still be a module-level named constant carrying a one-line rationale for *why that value* — not a bare number or string buried in an expression. Magic values in the middle of logic are forbidden regardless of how obvious they seem at the time of writing.

   Hardcoding is acceptable **only** when it is the sole workable option, and then it must be declared as a tradeoff under §11.2 and logged under §11.3. Prefer a small, auditable derivation over a convenient literal every time; a value that looks stable today is the one that silently diverges from its real source tomorrow. This applies to production code, tests, and prompts alike — an assertion that hardcodes an expected value it could derive is the same defect, and it is how a test keeps passing while the behaviour it guards has moved.
