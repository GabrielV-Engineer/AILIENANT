...
Covers, per the Phase 12.6 plan:
  ...
  PTY1     — an interactive session's lease is released exactly once on close.
...
class _FakeSocket:
    ...
    ``recv`` returns EOF immediately so the PTY reader thread a session's
    ``start()`` spawns exits promptly instead of blocking on a real fd — tests
    that open an interactive session need no further wiring for this to work.
    """
...
class _FakeAPI:
    ...
    def __init__(self, exit_code: int = 0, stdout: bytes = b"ok", stderr: bytes = b"") -> None:
